...
from model_gmm3_frontdoor_hsic_ebr_gumbel import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--lambda_ind', type=float, default=0.0,
                        help='legacy pointwise orthogonality weight; HSIC is used by default')
    parser.add_argument('--lambda_dag', type=float, default=0.0,
                        help='disabled by default in the explicit front-door DAG variant')
    parser.add_argument('--lambda_med', type=float, default=0.5,
                        help='weight of mediator-only supervised loss for M -> Y')
    ...
    parser.add_argument('--disable_cita_edge_similarity', action='store_false', dest='use_cita_edge_similarity',
                        help='disable the CITA-style cosine similarity prior in direct edge weighting')
    parser.set_defaults(use_cita_edge_similarity=True)
    parser.add_argument('--cita_edge_sim_weight', type=float, default=1.0,
                        help='weight of the CITA-style center-neighbor similarity prior in edge logits')
    parser.add_argument('--cita_edge_mlp_weight', type=float, default=1.0,
                        help='weight of the selected edge_feat_mode MLP score in edge logits')
    parser.add_argument('--disable_edge_score_norm', action='store_false', dest='use_edge_score_norm',
                        help='disable per-destination neighbour softmax over learned edge scores')
    parser.set_defaults(use_edge_score_norm=True)
    parser.add_argument('--edge_norm_temp', type=float, default=1.0,
                        help='temperature for per-destination edge-score neighbour normalization')
    parser.add_argument('--disable_edge_norm_degree', action='store_false', dest='edge_norm_with_degree',
                        help='disable multiplying normalized edge scores by GCN degree coefficient')
    parser.set_defaults(edge_norm_with_degree=True)
    ...
    parser.add_argument('--lambda_global_env', type=float, default=0.0,
                        help='disabled by default; not part of the core front-door objective')
    parser.add_argument('--lambda_hsic', type=float, default=0.05,
                        help='weight of HSIC(M, U) independence penalty between causal and spurious branches')
    parser.add_argument('--hsic_max_samples', type=int, default=2048,
                        help='maximum nodes sampled for HSIC; <=0 uses all nodes')
    parser.add_argument('--hsic_kernel', type=str, default='rbf', choices=['rbf', 'linear'],
                        help='kernel used by the HSIC independence loss')
    parser.add_argument('--hsic_sigma', type=float, default=-1.0,
                        help='RBF kernel bandwidth for HSIC; <=0 uses median heuristic')
    parser.add_argument('--lambda_ebr', type=float, default=0.05,
                        help='weight of CGRL-style energy-based reconstruction on causal adapter features')
    parser.add_argument('--ebr_temperature', type=float, default=1.0,
                        help='temperature for EBR edge energy logits')
    parser.add_argument('--ebr_neg_ratio', type=float, default=0.0,
                        help='negative edge ratio for optional EBR pairwise NCE/BCE; default 0 follows formula-(4) KL only')
    parser.add_argument('--ebr_max_edges', type=int, default=200000,
                        help='maximum positive edges sampled for EBR; <=0 uses all edges')
    parser.add_argument('--ebr_kl_weight', type=float, default=1.0,
                        help='weight of edge KL term KL(Q_phi(u,v|M)||P_theta(u,v)) inside EBR')
    parser.add_argument('--ebr_node_weight', type=float, default=1.0,
                        help='weight of CGRL node/Gumbel term KL(Q_phi(G_s|M)||P_theta(G_s)) inside EBR')
    parser.add_argument('--ebr_node_temperature', type=float, default=1.0,
                        help='temperature tau for CGRL node/Gumbel posterior Q_phi(G_s)')
    parser.add_argument('--disable_ebr_gumbel', action='store_false', dest='ebr_use_gumbel',
                        help='disable explicit Gumbel noise in the EBR node posterior and use softmax instead')
    parser.set_defaults(ebr_use_gumbel=True)
    parser.add_argument('--ebr_gumbel_eps', type=float, default=1e-10,
                        help='numerical epsilon for sampling g=-log(-log(u)) in the EBR node posterior')
    parser.add_argument('--lambda_proto_compact', type=float, default=0.0,
                        help='weight of mediator same-class prototype compactness loss')
    parser.add_argument('--lambda_proto_margin', type=float, default=0.0,
                        help='weight of mediator class-prototype margin loss')
    parser.add_argument('--proto_margin', type=float, default=1.0,
                        help='target distance margin between mediator class prototypes')
    parser.add_argument('--disable_proto_normalize', action='store_false', dest='proto_normalize',
                        help='disable L2 normalization before mediator prototype geometry loss')
    parser.set_defaults(proto_normalize=True)
    parser.add_argument('--ebr_nce_weight', type=float, default=0.0,
                        help='optional pairwise positive-vs-negative NCE inside EBR; disabled by default')
    parser.add_argument('--ebr_bce_weight', type=float, default=0.0,
                        help='optional BCE edge reconstruction weight inside EBR')
    parser.add_argument('--ebr_kl_direction', type=str, default='q_to_p', choices=['p_to_q', 'q_to_p'],
                        help='KL direction for EBR; q_to_p matches CGRL formula (4)')
    parser.add_argument('--ebr_prior', type=str, default='encoder', choices=['encoder', 'uniform'],
                        help='P_theta(G) target for EBR: detached encoder edge prior or uniform observed-neighbour prior')
    parser.add_argument('--disable_ebr_self_loop_removal', action='store_false', dest='ebr_remove_self_loops',
                        help='keep self-loops in EBR if present')
    parser.set_defaults(ebr_remove_self_loops=True)
    parser.add_argument('--disable_spu_env_context_save', action='store_false', dest='save_spu_env_contexts',
                        help='disable saving spurious adapter environment context prototypes')
    parser.set_defaults(save_spu_env_contexts=True)
...
parser = argparse.ArgumentParser(description='Graph Front-Door HSIC-EBR Training Pipeline')
...
log_dir = os.path.join('.', 'runs', args.dataset, 'frontdoor_hsic_ebr', run_name)
os.makedirs(log_dir, exist_ok=True)
writer = SummaryWriter(log_dir=log_dir)
print(f"[INFO] TensorBoard logging activated. Logs will be saved to: {log_dir}")
print(
        f"[INFO] Training recipe: explicit front-door HSIC+EBR | CIPT schedule: {args.use_cipt_schedule} | "
        ...
        f"CITA edge sim: {args.use_cita_edge_similarity} "
        f"(sim_w={args.cita_edge_sim_weight}, mlp_w={args.cita_edge_mlp_weight}) | "
    ...
    f"edge score norm: {args.use_edge_score_norm} (temp={args.edge_norm_temp}, "
    f"degree={args.edge_norm_with_degree}) | "
    f"HSIC: lambda={args.lambda_hsic}, kernel={args.hsic_kernel}, max={args.hsic_max_samples} | "
    f"EBR: lambda={args.lambda_ebr}, edge_temp={args.ebr_temperature}, "
    f"node_w={args.ebr_node_weight}, node_tau={args.ebr_node_temperature}, "
    f"gumbel={args.ebr_use_gumbel}, prior={args.ebr_prior}, "
    f"edge_kl={args.ebr_kl_weight}/{args.ebr_kl_direction}, nce={args.ebr_nce_weight}, "
    f"bce={args.ebr_bce_weight}, neg_ratio={args.ebr_neg_ratio}, "
    f"max_edges={args.ebr_max_edges} | "
    f"save spu contexts: {args.save_spu_env_contexts}"
)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Loss/HSIC', (model.lambda_hsic * losses['loss_hsic']).item(), global_step)
        ...
        writer.add_scalar('Loss/EBR', (model.lambda_ebr * losses['loss_ebr']).item(), global_step)
        writer.add_scalar('Loss/EBR_BCE', losses['loss_ebr_bce'].item(), global_step)
        writer.add_scalar('Loss/EBR_NCE', losses['loss_ebr_nce'].item(), global_step)
        writer.add_scalar('Loss/EBR_KL', losses['loss_ebr_kl'].item(), global_step)
        if 'loss_ebr_node' in losses:
            writer.add_scalar('Loss/EBR_NodeGumbel', losses['loss_ebr_node'].item(), global_step)
        if 'loss_ebr_edge' in losses:
            writer.add_scalar('Loss/EBR_EdgeKL', losses['loss_ebr_edge'].item(), global_step)
        writer.add_scalar('Loss/ProtoCompact', (model.lambda_proto_compact * losses['loss_proto_compact']).item(), global_step)
        writer.add_scalar('Loss/ProtoMargin', (model.lambda_proto_margin * losses['loss_proto_margin']).item(), global_step)
        writer.add_scalar('Graph/ProtoDistMean', losses['proto_dist_mean'].item(), global_step)
        writer.add_scalar('Graph/ProtoDistMin', losses['proto_dist_min'].item(), global_step)
        writer.add_scalar('Graph/EBRPosScore', losses['ebr_pos_score'].item(), global_step)
        writer.add_scalar('Graph/EBRNegScore', losses['ebr_neg_score'].item(), global_step)
        writer.add_scalar('Graph/EBRNumPos', losses['ebr_num_pos'].item(), global_step)
        writer.add_scalar('Graph/EBRNumNeg', losses['ebr_num_neg'].item(), global_step)
        writer.add_scalar('Graph/SavedSpuContexts', losses['saved_spu_contexts'].item(), global_step)
        if 'num_saved_contexts' in losses:
            writer.add_scalar('Graph/NumSavedContexts', losses['num_saved_contexts'].item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"HSIC: {(model.lambda_hsic * losses['loss_hsic']).item():.4f}, "
                ...
                f"EBR: {(model.lambda_ebr * losses['loss_ebr']).item():.4f}, "
                f"EBR_KL: {losses['loss_ebr_kl'].item():.4f}, "
                f"EBR_Node: {losses.get('loss_ebr_node', losses['loss_ebr_kl']).item():.4f}, "
                f"EBR_Edge: {losses.get('loss_ebr_edge', losses['loss_ebr_kl']).item():.4f}, "
                f"EBR_NCE: {losses['loss_ebr_nce'].item():.4f}, "
                f"ProtoC: {(model.lambda_proto_compact * losses['loss_proto_compact']).item():.4f}, "
                f"ProtoM: {(model.lambda_proto_margin * losses['loss_proto_margin']).item():.4f}, "
                f"ProtoMin: {losses['proto_dist_min'].item():.3f}, "
                f"SavedSpuCtx: {int(losses['saved_spu_contexts'].item())}, "
