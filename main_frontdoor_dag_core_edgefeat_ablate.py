...
from model_frontdoor_dag_core_edgefeat_ablate import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--edge_feat_mode', type=str, default='full', choices=['full', 'mul', 'diff', 'mul_diff'],
                        help=(
                            "edge scoring feature ablation: 'full' uses original "
                            "[h_src,h_dst,h_src*h_dst,|h_src-h_dst|]; 'mul' uses only h_src*h_dst; "
                            "'diff' uses only |h_src-h_dst|; 'mul_diff' uses only the interaction and difference terms"
                        ))
...
print(
    ...
    f"DAG mixer: {args.use_dag_mixer} | EdgeFeat: {args.edge_feat_mode} | "
