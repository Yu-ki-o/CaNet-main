...
from model_frontdoor_ciw_dag_stablestruct import GraphFrontDoorCIWStableStructDAG
...
def add_stable_struct_args(parser):
    """Arguments for StableGNN-inspired neighborhood structure variables."""
    ...
    parser.add_argument('--num_struct_tokens', type=int, default=4,
                        help='number of node-centric neighborhood structural tokens per node')
    ...
    parser.add_argument('--struct_factor_scale', type=float, default=0.0,
                        help='scale applied to structure factor; <=0 uses sqrt(non_label_dim / struct_dim)')
    parser.add_argument('--lambda_struct_aux', type=float, default=0.02,
                        help='auxiliary supervised loss weight for DAG-masked StableStruct readout')
    parser.add_argument('--lambda_struct_token_div', type=float, default=0.0,
                        help='optional weak diversity loss over the K structure tokens; keep 0 unless token collapse is observed')
    parser.add_argument('--struct_readout_blend', type=float, default=0.15,
                        help='blend ratio of DAG-masked StableStruct logits into final prediction logits')
...
model = GraphFrontDoorCIWStableStructDAG(d, c, args, device).to(device)
...
log_dir = os.path.join('.', 'runs', args.dataset, 'frontdoor_ciw_dag_stablestruct', run_name)
os.makedirs(log_dir, exist_ok=True)
writer = SummaryWriter(log_dir=log_dir)
print(f"[INFO] TensorBoard logging activated. Logs will be saved to: {log_dir}")
print(
    f"[INFO] Training recipe: CIW-DAG + FrontDoor + StableStruct | CIPT schedule: {args.use_cipt_schedule} | "
...
print(
    f"[INFO] StableStruct: enabled={args.use_stable_struct} | tokens={args.num_struct_tokens} | "
    f"inject_alpha={args.struct_inject_alpha} | lambda_aux={args.lambda_struct_aux} | "
    f"readout_blend={args.struct_readout_blend} | factor_scale={args.struct_factor_scale}"
)
