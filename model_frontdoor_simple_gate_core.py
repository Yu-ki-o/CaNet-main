...
class GraphFrontDoorDAG(nn.Module):
    """
    Front-door graph model with a lightweight fixed-path causal gate.

    Main idea:
    1) Encode nodes with the GNN backbone.
    2) Build a NodeIGM-style edge semantic summary from endpoint hidden states.
    3) Project node/edge summaries into a compact front-door bottleneck.
    4) Score mediator dimensions by label relevance minus edge/env pollution.
    ...
    - keeps edge/neighbor semantics as a pollution signal for mediator discovery,
    - replaces the learned DAG and matrix-exponential acyclicity constraint with
      a fixed front-door causal template:

          node latent -> label
          edge/env latent -> pollution -> mediator gate
          mediator -> label
          spurious -> context -> label

      This preserves the front-door decomposition while making the structural
      module much smaller and easier to optimize.
    """
    ...
    def __init__(self, d_in, c, args, device):
        ...
        self.causal_edge_summary_norm = nn.LayerNorm(self.d)
        self.env_edge_summary_norm = nn.LayerNorm(self.d)
        ...
        self.spurious_edge_norm = nn.LayerNorm(self.d)
        ...
        # Fixed-path causal gate parameters. They replace the full learnable DAG:
        # node/edge latents predict labels directly, node-node support captures
        # cooperative mediator groups, and edge-to-node pollution demotes
        # environment-sensitive shortcuts.
        self.causal_label_weight = Parameter(torch.zeros(self.non_label_var_dim, self.c))
        self.node_support_weight = Parameter(torch.zeros(self.dag_latent_dim, self.dag_latent_dim))
        self.edge_pollution_weight = Parameter(torch.zeros(self.dag_latent_dim, self.dag_latent_dim))
        # Learnable base score per bottleneck dimension; fixed-path structure refines it.
    ...
    def reset_parameters(self):
        ...
        self.causal_edge_summary_norm.reset_parameters()
        self.env_edge_summary_norm.reset_parameters()
        ...
        self.spurious_edge_norm.reset_parameters()
        ...
        nn.init.xavier_uniform_(self.causal_label_weight, gain=0.1)
        nn.init.xavier_uniform_(self.node_support_weight, gain=0.1)
        nn.init.xavier_uniform_(self.edge_pollution_weight, gain=0.1)
    ...
    def _zero_loss(self):
        return self.gate_base.new_zeros(())
    ...
    def get_causal_effect_and_mask(self):
        """
        Use a fixed-path causal gate to produce mediator scores.

        - label_effect: direct node-bottleneck relevance to label sinks.
        - causal_support: incoming support from other high-label-effect node
          dimensions, approximating mediator groups without a full DAG.
        - pollution_score: pressure from low-effect nodes and edge-summary
          dimensions that may encode environmental shortcuts.
        - gate_base: lets the task losses directly refine mediator selection,
          while the fixed front-door template acts as a structured prior.
        """
        dtype = self.causal_label_weight.dtype
        device = self.causal_label_weight.device

        label_weight = self.causal_label_weight
        node_to_label = label_weight[self.node_var_slice].abs()
        ...
        node_flow = F.softplus(self.node_support_weight)
        node_flow = node_flow * (1.0 - torch.eye(self.node_var_dim, device=device, dtype=dtype))
        edge_to_node = F.softplus(self.edge_pollution_weight)
        edge_sensitivity = self.edge_env_sensitivity.to(device=device, dtype=dtype)
        if edge_sensitivity.sum() > 1e-6:
            edge_weight = edge_sensitivity / edge_sensitivity.sum().clamp_min(1e-8)
            edge_incoming = torch.matmul(edge_to_node, edge_weight)
        else:
            edge_incoming = torch.zeros(self.node_var_dim, device=device, dtype=dtype)
        ...
        # Compatibility tensor for logging/loss code that expects a DAG-shaped
        # total-flow matrix. It encodes only the fixed front-door template.
        flow = torch.zeros(self.dag_var_dim, self.dag_var_dim, device=device, dtype=dtype)
        flow[self.node_var_slice, self.node_var_slice] = node_flow
        flow[self.edge_var_slice, self.node_var_slice] = edge_to_node.t()
        flow[:self.non_label_var_dim, self.label_var_slice] = label_weight.abs()
        eye = torch.eye(self.dag_var_dim, device=device, dtype=dtype)
        causal_total = flow + eye
        return causal_score, pollution_score, mediator_gate, causal_total

    def compute_edge_semantic_summaries(self, h, edge_index, training=False):
        if edge_index.numel() == 0:
            zero = h.new_zeros(h.size())
            return zero, zero, h.new_zeros(0)
        ...
        causal_edge_score = torch.sigmoid(edge_logits)
        env_edge_score = 1.0 - causal_edge_score

        deg = degree(dst, h.size(0)).to(device=h.device, dtype=h.dtype).clamp_min(1.0)
        norm = deg[src].pow(-0.5) * deg[dst].pow(-0.5)
        causal_edge_weight = torch.nan_to_num(
            norm * causal_edge_score,
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        )
        env_edge_weight = torch.nan_to_num(
            norm * env_edge_score,
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        ...
        causal_edge_summary = h.new_zeros(h.size())
        env_edge_summary = h.new_zeros(h.size())
        causal_edge_summary.index_add_(0, dst, causal_edge_weight.unsqueeze(-1) * h[src])
        env_edge_summary.index_add_(0, dst, env_edge_weight.unsqueeze(-1) * h[src])
        causal_edge_summary = self.causal_edge_summary_norm(causal_edge_summary)
        env_edge_summary = self.env_edge_summary_norm(env_edge_summary)
        return causal_edge_summary, env_edge_summary, causal_edge_score

    def compute_edge_semantic_summary(self, h, edge_index, training=False):
        causal_edge_summary, _, _ = self.compute_edge_semantic_summaries(
            h,
            edge_index,
            training=training,
        )
        return causal_edge_summary
    ...
    def inject_environment_edges(self, z_spurious, env_edge_summary, training=False):
        env_edge_summary = F.dropout(env_edge_summary, self.dropout, training=training)
        return self.spurious_edge_norm(z_spurious + self.edge_blend * env_edge_summary)
    ...
    def encode_representation(self, x, edge_index, training=False):
        ...
        causal_edge_summary, env_edge_summary, _ = self.compute_edge_semantic_summaries(
            h,
            edge_index,
            training=training,
        )
        z = self.fuse_node_edge_representation(h, causal_edge_summary, training=training)
        node_latent = self.node_dag_proj(z)
        edge_latent = self.edge_dag_proj(env_edge_summary)
        ...
        z_spurious = self.inject_environment_edges(z_spurious, env_edge_summary, training=training)
    ...
    def dag_regularization_loss(self, mediator_gate, dag_total):
        structural_l1 = (
            self.causal_label_weight.abs().mean()
            + self.node_support_weight.abs().mean()
            + self.edge_pollution_weight.abs().mean()
        )
        loss_dag = self.lambda_l1 * structural_l1
    ...
    def dag_label_loss(self, dag_vars, labels, train_idx, criterion, args):
        """
        Direct supervision for the fixed-path causal label weights.
        ...
        label_logits = torch.matmul(dag_vars, self.causal_label_weight) + self.dag_label_bias
    ...
    def compute_counterexample_penalty(self, dag_vars, labels, train_idx, criterion, args):
        ...
        label_A = self.causal_label_weight[self.node_var_slice].detach()
    ...
    def dag_semantic_loss(self, dag_vars, labels, train_idx, criterion, args):
        # Deprecated in Simple-Gate. Kept for backward-compatible logging/calls.
        return self._zero_loss()
