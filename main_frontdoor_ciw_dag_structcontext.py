...
from model_frontdoor_ciw_dag_structcontext import GraphFrontDoorCIWStructContextDAG
...
def add_ciw_dag_args(parser):
    """Arguments for article-aligned CIW DAG construction."""
    parser.add_argument('--lambda_dag_rec', type=float, default=0.05,
                        help='weight of CIW Eq.(1)-(2) DAG reconstruction loss')
    parser.add_argument('--lambda_dag_feat', type=float, default=1.0,
                        help='relative weight of feature-factor L2 reconstruction inside L_rec')
    parser.add_argument('--lambda_dag_label_rec', type=float, default=1.0,
                        help='relative weight of label-factor CE reconstruction inside L_rec')
    parser.add_argument('--lambda_dag_proto', type=float, default=0.0,
                        help='optional prototype DAG reconstruction weight inside L_rec')
    parser.add_argument('--dag_proto_min_count', type=int, default=1,
                        help='minimum nodes needed to form an env/class DAG prototype')
    parser.add_argument('--use_dag_feature_mlp', action='store_true',
                        help='use a small MLP on parent signals for feature reconstruction; default follows Eq.(1) directly')
    parser.add_argument('--dag_rec_detach_input', action='store_true',
                        help='detach factors inside CIW DAG reconstruction so L_rec mainly trains A and M')
    parser.add_argument('--dag_rec_signed_mask', action='store_false', dest='dag_rec_use_abs_mask',
                        help='use signed A-to-label mask in M(A^T odot F) instead of abs-normalized parent strength')
    parser.set_defaults(dag_rec_use_abs_mask=True)
    parser.add_argument('--use_current_spurious_in_fd', action='store_false', dest='zero_current_spurious_in_fd',
                        help='allow current-node spurious representation in DAG mixer; default zeros it to preserve front-door intervention')
    parser.set_defaults(zero_current_spurious_in_fd=True)
    parser.add_argument('--eval_gmm_sample_noise', action='store_false', dest='eval_gmm_use_mean',
                        help='sample GMM noise at evaluation; default uses deterministic GMM means')
    parser.set_defaults(eval_gmm_use_mean=True)
    parser.add_argument('--lambda_causal_mask_cls', type=float, default=0.0,
                        help='optional auxiliary classification through raw causal-effect mask; off by default')



def add_stable_struct_args(parser):
    """Arguments for StableGNN-inspired neighborhood variables used inside DAG + front-door."""
    parser.add_argument('--disable_stable_struct', action='store_false', dest='use_stable_struct',
                        help='disable StableStruct neighborhood variable injection into DAG edge/structure slice')
    parser.set_defaults(use_stable_struct=True)
    parser.add_argument('--disable_struct_context', action='store_false', dest='use_struct_context',
                        help='do not fuse DAG-polluted structural variables into spurious environment context sampling')
    parser.set_defaults(use_struct_context=True)
    parser.add_argument('--num_struct_tokens', type=int, default=4,
                        help='number of node-centric neighborhood structural tokens per node; must divide edge/structure DAG slice width')
    parser.add_argument('--struct_hidden_dim', type=int, default=0,
                        help='dimension of z_all consumed by StableStructEncoder; 0 uses hidden_channels')
    parser.add_argument('--struct_context_hidden_dim', type=int, default=0,
                        help='dimension of z_spurious/env context; 0 uses hidden_channels')
    parser.add_argument('--struct_inject_alpha', type=float, default=0.5,
                        help='maximum replacement strength when injecting StableStruct variables into DAG structure slice')
    parser.add_argument('--struct_factor_scale', type=float, default=0.0,
                        help='scale applied to flattened structure tokens; <=0 uses sqrt(non_label_dim / struct_dim)')
    parser.add_argument('--struct_spu_threshold', type=float, default=0.5,
                        help='threshold for selecting DAG-polluted structural variables as environment context')
    parser.add_argument('--struct_spu_temp', type=float, default=8.0,
                        help='temperature for DAG-pollution structural spurious mask')
    parser.add_argument('--lambda_struct_aux', type=float, default=0.0,
                        help='optional very weak supervised structure loss; default 0 keeps DAG/front-door as the main frame')
    parser.add_argument('--lambda_struct_token_div', type=float, default=0.0,
                        help='optional weak diversity loss over K structure tokens; default 0')
    parser.add_argument('--lambda_struct_env', type=float, default=0.0,
                        help='optional extra pseudo-env loss on fused node+struct spurious context; default 0')
...
def apply_cipt_schedule(model, base_lambdas, epoch, args):
    ...
    if 'lambda_dag_rec' in base_lambdas:
        setattr(model, 'lambda_dag_rec', base_lambdas['lambda_dag_rec'] * dag_scale)
...
parser = argparse.ArgumentParser(description='Graph Front-Door CIW-DAG + StableStruct Training Pipeline')
parser_add_main_args(parser)
add_frontdoor_dag_args(parser)
add_ciw_dag_args(parser)
add_stable_struct_args(parser)
...
model = GraphFrontDoorCIWStructContextDAG(d, c, args, device).to(device)
...
if not run_name:
    run_name = (
        f"{current_time}_fd_ciw_dag_d{args.lambda_dag}_dl{args.lambda_dag_label}_fd{args.lambda_fd}"
        f"_warm{args.decomp_warmup_epochs}_ramp{args.intervention_ramp_epochs}"
    )
log_dir = os.path.join('.', 'runs', args.dataset, 'frontdoor_ciw_dag_structcontext', run_name)
os.makedirs(log_dir, exist_ok=True)
writer = SummaryWriter(log_dir=log_dir)
print(f"[INFO] TensorBoard logging activated. Logs will be saved to: {log_dir}")
print(
    f"[INFO] Training recipe: CIW-DAG + FrontDoor + StableStructContext | CIPT schedule: {args.use_cipt_schedule} | "
    ...
    f"DAG mixer: {args.use_dag_mixer} | CIW Lrec: {args.lambda_dag_rec} | zero current spurious in FD: {args.zero_current_spurious_in_fd} | GMM contexts: {args.use_spu_gmm} | "
...
print(
    f"[INFO] StableStructContext: struct_enabled={args.use_stable_struct} | context_enabled={args.use_struct_context} | "
    f"tokens={args.num_struct_tokens} | inject_alpha={args.struct_inject_alpha} | "
    f"lambda_aux={args.lambda_struct_aux} | lambda_struct_env={args.lambda_struct_env} | "
    f"spu_threshold={args.struct_spu_threshold} | factor_scale={args.struct_factor_scale}"
)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Loss/DAGLabel', (getattr(model, 'lambda_dag_rec', model.lambda_dag_label) * losses['loss_dag_label']).item(), global_step)
        if 'loss_dag_feat' in losses:
            writer.add_scalar('Loss/DAGFeatureRec', losses['loss_dag_feat'].item(), global_step)
        if 'loss_dag_label_rec' in losses:
            writer.add_scalar('Loss/DAGLabelRecRaw', losses['loss_dag_label_rec'].item(), global_step)
        ...
        if 'loss_struct_aux' in losses:
            writer.add_scalar('Loss/StructAux', (getattr(model, 'lambda_struct_aux', 0.0) * losses['loss_struct_aux']).item(), global_step)
        if 'loss_struct_token_div' in losses:
            writer.add_scalar('Loss/StructTokenDiv', (getattr(model, 'lambda_struct_token_div', 0.0) * losses['loss_struct_token_div']).item(), global_step)
        if 'loss_struct_env' in losses:
            writer.add_scalar('Loss/StructEnv', (getattr(model, 'lambda_struct_env', 0.0) * losses['loss_struct_env']).item(), global_step)
        if 'struct_assign_entropy' in losses:
            writer.add_scalar('Struct/AssignEntropy', losses['struct_assign_entropy'].item(), global_step)
        if 'struct_mix_mean' in losses:
            writer.add_scalar('Struct/MixMean', losses['struct_mix_mean'].item(), global_step)
        if 'struct_factor_norm' in losses:
            writer.add_scalar('Struct/FactorNorm', losses['struct_factor_norm'].item(), global_step)
        if 'struct_spu_mask_mean' in losses:
            writer.add_scalar('Struct/SpuMaskMean', losses['struct_spu_mask_mean'].item(), global_step)
        if 'struct_spu_context_norm' in losses:
            writer.add_scalar('Struct/SpuContextNorm', losses['struct_spu_context_norm'].item(), global_step)
        if 'env_context_norm' in losses:
            writer.add_scalar('EnvContext/Norm', losses['env_context_norm'].item(), global_step)
        if 'dag_node_label_strength' in losses:
            writer.add_scalar('DAG/NodeToLabelStrength', losses['dag_node_label_strength'].item(), global_step)
        if 'dag_struct_label_strength' in losses:
            writer.add_scalar('DAG/StructToLabelStrength', losses['dag_struct_label_strength'].item(), global_step)
        if 'dag_struct_pollution_strength' in losses:
            writer.add_scalar('DAG/StructPollutionStrength', losses['dag_struct_pollution_strength'].item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"StructAux: {(getattr(model, 'lambda_struct_aux', 0.0) * losses.get('loss_struct_aux', losses['loss_cls'].new_zeros(())).item()):.4f}, "
                f"LR: {current_lr:.6f}, "
