...
from model_frontdoor_ori import GraphFrontDoor
...
def add_frontdoor_args(parser):
    ...
    parser.add_argument('--use_spu_gmm', action='store_true',
                        help='enable GMM sampling from spurious environment contexts')
    parser.add_argument('--use_soft_pseudo_gmm', action='store_true',
                        help='fit GMM stats with soft pseudo-environment probabilities from the spurious branch')
    parser.add_argument('--lambda_pseudo_env', type=float, default=0.05,
                        help='weight of the soft pseudo-environment discovery loss used by soft pseudo GMM')
    parser.add_argument('--pseudo_env_balance', type=float, default=1.0,
                        help='balance term weight inside the pseudo-environment discovery loss')
    parser.add_argument('--gmm_sample_k', type=int, default=0,
                        help='number of GMM-sampled spurious contexts; <=0 falls back to K/fd_sample_k')
    parser.add_argument('--gmm_min_var', type=float, default=1e-4,
                        help='minimum diagonal variance for spurious-context GMM')
    ...
    parser.add_argument('--edge_feat_mode', type=str, default='mul_diff',
                        ...
                        help='edge feature mode for optional node representation enhancement')
    parser.add_argument('--edge_score_temp', type=float, default=2.0,
                        help='temperature for optional edge semantic gates')
    parser.add_argument('--edge_blend', type=float, default=0.0,
                        help='residual strength of optional edge-aware node representation enhancement')
    parser.add_argument('--use_neighbor_denoise', action='store_true',
                        help='softly subtract low-relevance neighbor summary in the node enhancement module')
    parser.add_argument('--noise_subtract_alpha', type=float, default=0.1,
                        help='strength for subtracting low-relevance neighbor component')
    parser.add_argument('--noise_gate_temp', type=float, default=1.0,
                        help='temperature for the node-level denoising gate')
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Loss/PseudoEnv', (model.lambda_pseudo_env * losses.get('loss_pseudo_env', torch.tensor(0.0, device=device))).item(), global_step)
        ...
        writer.add_scalar('Graph/NumGMMContexts', losses.get('num_gmm_contexts', torch.tensor(0.0)).item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"PseudoEnv: {(model.lambda_pseudo_env * losses.get('loss_pseudo_env', torch.tensor(0.0, device=device))).item():.4f}, "
                f"Ctx: {int(losses['num_contexts'].item())}, "
                f"GMMCtx: {int(losses.get('num_gmm_contexts', torch.tensor(0.0)).item())}, "
