...
def get_last_final_ood_metric(record):
    ...
    last_ood_key = f"final_ood_test_{max(ood_indices)}"
    return record.get(last_ood_key)
...
def write_topk_summary(results, summary_path, dataset, stage, search_keys):
    ranked_results = sorted(
        results,
        key=lambda item: (
            safe_metric_value(item, "last_final_ood_test"),
    ...
    with open(summary_path, "w") as summary_file:
        summary_file.write(
            f"Top {len(top_results)} experiments on {dataset} ({stage}, MLEI-GAT) "
        ...
        for rank, result in enumerate(top_results, start=1):
            ...
            summary_file.write(f"rank_metric ({RANK_METRIC}): {result.get('last_final_ood_test')}\n")
            summary_file.write(f"highest_valid: {result.get('highest_valid')} ± {result.get('highest_valid_std')}\n")
            summary_file.write(f"highest_in_test: {result.get('highest_in_test')} ± {result.get('highest_in_test_std')}\n")
            summary_file.write(f"final_in_test: {result.get('final_in_test')} ± {result.get('final_in_test_std')}\n")
            ...
            if final_ood_keys:
                ...
                for key in final_ood_keys:
                    summary_file.write(f"  {key}: {result.get(key)} ± {result.get(f'{key}_std')}\n")
            summary_file.write("\n")


def run_grid_search(dataset, base_cmd, param_grid, search_stage, search_root="search_logs_MLEI"):
    keys = list(param_grid.keys())
    values = list(param_grid.values())
    combinations = list(itertools.product(*values))

    log_dir = os.path.join(search_root, dataset, "gat", search_stage)
    os.makedirs(log_dir, exist_ok=True)

    all_results = []

    print(f"Start {search_stage} grid search on {dataset} (MLEI-GAT), total runs: {len(combinations)}")
    print("-" * 80)
    ...
    for idx, combination in enumerate(combinations, start=1):
        ...
        result_name_parts = [dataset, "mlei", "gat", search_stage]
        ...
        record["last_final_ood_test"] = get_last_final_ood_metric(record)
        ...
        all_results_csv = os.path.join(log_dir, f"{dataset}_mlei_gat_{search_stage}_all_results.csv")
        all_results_json = os.path.join(log_dir, f"{dataset}_mlei_gat_{search_stage}_all_results.json")
        topk_summary = os.path.join(log_dir, f"{dataset}_mlei_gat_{search_stage}_top{TOP_K}.txt")

        write_all_results_csv(all_results, all_results_csv)
        with open(all_results_json, "w") as json_file:
            json.dump(all_results, json_file, indent=2)
        write_topk_summary(all_results, topk_summary, dataset, search_stage, keys)
