...
class GraphFrontDoorDAG(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        self.layerwise_gate_target = float(getattr(args, 'layerwise_gate_target', 0.5))
        self.lambda_layerwise_gate = float(getattr(args, 'lambda_layerwise_gate', 0.0))
        # Anchor + orthogonal decomposition controls. The invariant score is
        # dominated by a fixed edge identity anchor, while dynamic context is
        # allowed only as a bounded correction. The environment score uses the
        # orthogonal residual as context for front-door adjustment.
        self.anchor_orth_lambda = min(max(float(getattr(args, 'anchor_orth_lambda', 0.2)), 0.0), 0.49)
        self.anchor_orth_perp_scale = max(1e-3, float(getattr(args, 'anchor_orth_perp_scale', 1.0)))
        # FD-edge routing controls. In fd_edge mode, edge weights are
        # dominated by a fixed anchor relation q(r_dst)^T k(r_src), while the
        # current layer relation is allowed only as a bounded correction. This
        # prevents a source node that has absorbed a causal neighbor from
        # washing a weak base edge into a high invariant edge at later layers.
        lambda_dynamic = getattr(args, 'lambda_dynamic', None)
        if lambda_dynamic is None:
            lambda_dynamic = getattr(args, 'stable_qk_lambda', 0.2)
        self.lambda_dynamic = min(max(float(lambda_dynamic), 0.0), 0.49)
        self.stable_qk_env_margin = max(0.0, float(getattr(args, 'stable_qk_env_margin', 0.0)))
        self.stable_qk_env_scale = max(0.0, float(getattr(args, 'stable_qk_env_scale', 1.0)))
        env_alpha = getattr(args, 'env_alpha', None)
        if env_alpha is None:
            env_alpha = getattr(args, 'edge_context_alpha', 0.0)
        self.env_alpha = max(0.0, float(env_alpha))
        self.edge_context_include_final = bool(getattr(args, 'edge_context_include_final', True))
        # Optional low-relevance neighbor denoising. With anchor_orth, the
        # denoised component is the environment-channel summary, which is also
        # available to the front-door context branch through env_alpha.
        ...
        self.stable_base_q = nn.Linear(self.d, self.d, bias=False)
        self.stable_base_k = nn.Linear(self.d, self.d, bias=False)
        self.stable_dyn_q = nn.Linear(self.d, self.d, bias=False)
        self.stable_dyn_k = nn.Linear(self.d, self.d, bias=False)
        self.stable_value_proj = nn.Linear(self.d, self.d, bias=False)
        self.stable_env_value_proj = nn.Linear(self.d, self.d, bias=False)
        ...
        self._last_layerwise_gate_layers = None
    ...
    def reset_parameters(self):
        ...
        for module in (
            ...
            self.stable_dyn_q,
            self.stable_dyn_k,
            self.stable_value_proj,
            self.stable_env_value_proj,
        ):
            nn.init.xavier_uniform_(module.weight)
    ...
    def _get_edge_feat_dim(self, mode):
        ...
        if mode == 'degree':
            return 1
        if mode in ('mul_diff', 'concat', 'anchor_orth', 'stable_qk', 'fd_edge'):
            return 2 * self.d if mode not in ('anchor_orth', 'stable_qk', 'fd_edge') else 3
        if mode == 'concat_diff':
            return 3 * self.d
        ...
        raise ValueError(
            ...
            "mul, diff, degree, mul_diff, concat, concat_diff, anchor_orth, fd_edge, stable_qk, "
    ...
    def build_edge_feat(self, h_src, h_dst, deg_src, deg_dst, deg_max):
        ...
        if mode in ('anchor_orth', 'fd_edge', 'stable_qk'):
            # Placeholder for module initialization compatibility. The actual
            # anchor_orth / fd_edge gates are computed analytically rather
            # than by edge_pair_encoder.
            return torch.cat([mul_feat.mean(dim=-1, keepdim=True),
                              diff_feat.mean(dim=-1, keepdim=True),
                              deg_pair], dim=-1)
        raise ValueError(
            f"Unknown edge_feat_mode='{mode}'. Use one of: "
            "mul, diff, degree, mul_diff, concat, concat_diff, anchor_orth, fd_edge, stable_qk, "
    ...
    def _cos01(self, x, y):
        return 0.5 * (F.cosine_similarity(x, y, dim=-1, eps=1e-8) + 1.0)

    def decompose_parallel_perp(self, h, anchor):
        denom = anchor.pow(2).sum(dim=-1, keepdim=True).clamp_min(1e-8)
        coeff = (h * anchor).sum(dim=-1, keepdim=True) / denom
        h_parallel = coeff * anchor
        h_perp = h - h_parallel
        return h_parallel, h_perp

    def compute_anchor_orth_gates(self, h_src, h_dst, anchor_src, anchor_dst):
        """
        Anchor + orthogonal decomposition relation operator.

        base_gate depends only on stable endpoint anchors and therefore remains
        close to the original u-v relation even if the source representation has
        absorbed another node. The dynamic projected relation gives a bounded
        correction. The orthogonal residual is interpreted as environment
        context and is routed through the env/noise channel.
        """
        h_src_parallel, h_src_perp = self.decompose_parallel_perp(h_src, anchor_src)
        base_gate = self._cos01(anchor_dst, anchor_src)
        proj_gate = self._cos01(anchor_dst, h_src_parallel)
        perp_ratio = h_src_perp.norm(dim=-1) / h_src.norm(dim=-1).clamp_min(1e-8)
        perp_gate = torch.sigmoid(self.anchor_orth_perp_scale * (perp_ratio - 0.5))

        lam = self.anchor_orth_lambda
        # Invariant gate is base-dominant. If the original edge identity is low,
        # dynamic context cannot wash it into a high invariant edge.
        inv_gate = base_gate * ((1.0 - lam) + lam * proj_gate * (1.0 - perp_gate))
        inv_gate = inv_gate.clamp(0.0, 1.0)

        # Environment gate is high when the current source is informative but the
        # stable endpoint relation is weak and/or the source carries an orthogonal
        # residual. It is used both for optional denoising and context routing.
        env_dynamic = torch.clamp(0.5 * proj_gate + 0.5 * perp_gate, 0.0, 1.0)
        env_gate = ((1.0 - base_gate) * env_dynamic).clamp(0.0, 1.0)
        return inv_gate, env_gate, base_gate, proj_gate, perp_gate

    def compute_stable_qk_weights(self, h_src, h_dst, anchor_src, anchor_dst, dst, num_nodes):
        """
        Stable Query-Key relation operator.

        The invariant routing weight is a convex mixture of:
          - base attention from stable anchors, q(r_dst)^T k(r_src)
          - dynamic attention from current states, q(h_dst)^T k(h_src)
        with lambda_dynamic < 0.5. Thus the edge identity remains closer to
        the original u-v relation than to a new relation induced by upstream
        messages absorbed by the source node.

        The environment routing weight is the positive excess of dynamic
        attention over base attention. It captures edges that become salient
        only after dynamic propagation, and routes that excess to the
        spurious/context branch rather than the mediator main stream.
        """
        scale = float(self.d) ** 0.5
        q_base = self.stable_base_q(anchor_dst)
        k_base = self.stable_base_k(anchor_src)
        q_dyn = self.stable_dyn_q(h_dst)
        k_dyn = self.stable_dyn_k(h_src)

        base_logits = (q_base * k_base).sum(dim=-1) / max(scale, 1e-6)
        dyn_logits = (q_dyn * k_dyn).sum(dim=-1) / max(scale, 1e-6)
        base_alpha = softmax(base_logits / self.edge_score_temp, dst, num_nodes=num_nodes)
        dyn_alpha = softmax(dyn_logits / self.edge_score_temp, dst, num_nodes=num_nodes)

        lam = self.lambda_dynamic
        inv_weight = (1.0 - lam) * base_alpha + lam * dyn_alpha
        env_weight = torch.clamp(dyn_alpha - base_alpha - self.stable_qk_env_margin, min=0.0)
        env_weight = self.stable_qk_env_scale * env_weight
        return inv_weight, env_weight, base_alpha, dyn_alpha

    def compute_edge_summaries(self, h, edge_index, training=False, anchor=None):
        """
        Build invariant/useful and environment summaries.

        In regular modes, edge_gate is produced by the historical MLP and the
        environment summary uses (1 - edge_gate). In anchor_orth mode, the
        invariant gate uses a base-dominant anchor relation, while the
        environment gate uses the orthogonal residual of the current source
        representation. The environment summary can be denoised from the main
        node stream and/or injected into the front-door spurious context.
        """
        ...
        h_src = h[src]
        h_dst = h[dst]
        if self.edge_feat_mode == 'anchor_orth':
            if anchor is None:
                anchor = h.detach()
            anchor_src = anchor[src]
            anchor_dst = anchor[dst]
            edge_gate, env_gate, _, _, _ = self.compute_anchor_orth_gates(
                h_src,
                h_dst,
                anchor_src,
                anchor_dst,
            ...
            useful_value = h_src
            noise_value = h_src
        elif self.edge_feat_mode in ('fd_edge', 'stable_qk'):
            if anchor is None:
                anchor = h.detach()
            anchor_src = anchor[src]
            anchor_dst = anchor[dst]
            useful_weight, noise_weight, base_alpha, dyn_alpha = self.compute_stable_qk_weights(
                h_src,
                h_dst,
                anchor_src,
                anchor_dst,
            ...
            useful_weight = torch.nan_to_num(useful_weight, nan=0.0, posinf=0.0, neginf=0.0)
            noise_weight = torch.nan_to_num(noise_weight, nan=0.0, posinf=0.0, neginf=0.0)
            useful_value = self.stable_value_proj(h_src)
            noise_value = self.stable_env_value_proj(h_src)
            edge_gate = useful_weight
        else:
        ...
        useful_summary.index_add_(0, dst, useful_weight.unsqueeze(-1) * useful_value)
    ...
    def compute_edge_semantic_summary(self, h, edge_index, training=False):
        ...
        useful_summary, _, _ = self.compute_edge_summaries(
            h, edge_index, training=training, anchor=h
        )
    ...
    def encode_representation(self, x, edge_index, training=False):
        # Stable anchor used by the anchor_orth relation operator. It is computed
        # from the raw input path and is reused at every GNN layer, so a source
        # node that absorbs a causal neighbor cannot wash a weak base edge into a
        # high invariant edge at later layers.
        h_anchor = self.act_fn(self.input_proj(x))
        h = F.dropout(x, self.dropout, training=training)
        h = self.act_fn(self.input_proj(h))

        layerwise_gate_values = []
        edge_context_accum = None
        used_layerwise_routes = 0
        for layer_idx, layer in enumerate(self.backbone_layers):
            h = F.dropout(h, self.dropout, training=training)
            h = self.act_fn(layer(h, edge_index))

            is_last_layer = layer_idx == len(self.backbone_layers) - 1
            should_route = self.use_layerwise_local_igm and not (
                self.layerwise_local_igm_skip_last and is_last_layer
            )
            if should_route:
                layer_edge_summary, layer_env_summary, layer_gate = self.compute_edge_summaries(
                    h,
                    edge_index,
                    training=training,
                    anchor=h_anchor,
                ...
                h = self.fuse_node_edge_representation(
                    h,
                    layer_edge_summary,
                    noise_summary=layer_env_summary,
                    training=training,
                )
                if edge_context_accum is None:
                    edge_context_accum = layer_env_summary
                else:
                    edge_context_accum = edge_context_accum + layer_env_summary
                if layer_gate is not None:
                    layerwise_gate_values.append(layer_gate.mean())
                used_layerwise_routes += 1

        if layerwise_gate_values:
            self._last_layerwise_gate_mean = torch.stack(layerwise_gate_values).mean()
            gate_budget = (self._last_layerwise_gate_mean - self.layerwise_gate_target) ** 2
            self._last_layerwise_gate_loss = gate_budget
            self._last_layerwise_gate_layers = h.new_tensor(float(len(layerwise_gate_values)))
        else:
            self._last_layerwise_gate_mean = h.new_tensor(0.0)
            self._last_layerwise_gate_loss = h.new_tensor(0.0)
            self._last_layerwise_gate_layers = h.new_tensor(0.0)
        ...
        edge_summary, noise_summary, final_edge_gate = self.compute_edge_summaries(
            h,
            edge_index,
            training=training,
            anchor=h_anchor,
        )
        if self.edge_context_include_final:
            if edge_context_accum is None:
                edge_context_accum = noise_summary
            else:
                edge_context_accum = edge_context_accum + noise_summary

        if self.layerwise_final_edge_fuse or not self.use_layerwise_local_igm:
        ...
        z_spurious_base = z * (1.0 - mediator_gate).unsqueeze(0)
        if self.env_alpha > 0.0 and edge_context_accum is not None:
            edge_context = self.edge_context_norm(edge_context_accum)
            z_spurious_base = z_spurious_base + self.env_alpha * edge_context
        z_spurious = F.dropout(z_spurious_base, self.dropout, training=training)
    ...
    def compute_losses(self, data, criterion, args, update_state=False):
        ...
        loss_inv = zero
        loss_layerwise_gate = (
            self._last_layerwise_gate_loss
            if self._last_layerwise_gate_loss is not None
            else zero
        )
        layerwise_gate_mean = (
            self._last_layerwise_gate_mean.detach()
            if self._last_layerwise_gate_mean is not None
            else zero.detach()
        )
        layerwise_gate_layers = (
            self._last_layerwise_gate_layers.detach()
            if self._last_layerwise_gate_layers is not None
            else zero.detach()
        )
        ...
        return {
            ...
            'layerwise_gate_mean': layerwise_gate_mean,
            'layerwise_gate_layers': layerwise_gate_layers,
