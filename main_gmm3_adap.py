...
from model_gmm3_adap import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--lambda_fd_aug', type=float, default=0.0,
                        help='deprecated in this hybrid: mixed-context extra supervision is disabled')
    parser.add_argument('--lambda_var', type=float, default=0.0,
                        help='deprecated in this hybrid: cross-context variance penalty is disabled')
    ...
    parser.add_argument('--lambda_inv', type=float, default=0.0,
                        help='deprecated in this hybrid: cross-environment prediction invariance is disabled')
    ...
    parser.add_argument('--lambda_sem', type=float, default=0.0,
                        help='deprecated in this hybrid: semantic reconstruction is removed')
    parser.add_argument('--lambda_spu_y', type=float, default=0.0,
                        help='deprecated in this hybrid: environment-conditioned spurious label supervision is disabled')
    parser.add_argument('--dag_latent_dim', type=int, default=16,
                        help='compatibility-only bottleneck dimension used for diagnostics/state payloads')
    ...
    parser.add_argument('--disable_layerwise_final_edge_fuse', action='store_false',
                        ...
                        help='keep final edge summary diagnostics but do not fuse it into z after layer-wise routing')
    ...
    parser.add_argument('--use_global_info', action='store_true',
                        help='inject MLEI/AdvDIFFormer-style global information before adapter/front-door splitting')
    ...
    parser.add_argument('--proto_aug_k', type=int, default=1,
                        help='deprecated: prototype mixup is replaced by GMM context sampling')
    ...
    parser.add_argument('--use_dag_mixer', action='store_true',
                        help='optionally use the legacy DAG-masked latent mixer instead of the CIPT concat fuser')
...
parser = argparse.ArgumentParser(description='Graph Front-Door CIPT-Adapter Training Pipeline')
...
if not run_name:
    run_name = (
        f"{current_time}_fd_cipt_adapters_fd{args.lambda_fd}"
        f"_warm{args.decomp_warmup_epochs}_ramp{args.intervention_ramp_epochs}"
    )
log_dir = os.path.join('.', 'runs', args.dataset, 'frontdoor_cipt_adapters', run_name)
os.makedirs(log_dir, exist_ok=True)
writer = SummaryWriter(log_dir=log_dir)
print(f"[INFO] TensorBoard logging activated. Logs will be saved to: {log_dir}")
print(
    f"[INFO] Training recipe: CIPT-Adapters + GMM/local-bi-smooth | "
    f"CIPT schedule: {args.use_cipt_schedule} | "
    ...
    f"legacy DAG mixer: {args.use_dag_mixer} | edge feat: {args.edge_feat_mode} | "
