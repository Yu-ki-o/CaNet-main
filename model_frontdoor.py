...
class GraphFrontDoor(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        self.lambda_spu_env = getattr(args, 'lambda_spu_env', 0.0)
    ...
    def update_spurious_env_prototypes(self, z_spurious, pseudo_envs):
    ...
    @torch.no_grad()
    def update_spurious_gmm(self, z_spurious, pseudo_envs):
        if not self.use_spu_gmm or pseudo_envs is None or pseudo_envs.numel() == 0:
            return
        env_values = pseudo_envs.squeeze().long()
        for env_idx in range(self.num_envs):
            mask_e = env_values == env_idx
    ...
    def sample_gmm_contexts(self, training=False):
        ...
        mean = self.gmm_spu_mean.index_select(0, env_indices)
        var = self.gmm_spu_var.index_select(0, env_indices).clamp_min(self.gmm_min_var)
        std = var.sqrt()
    ...
    def rl_context_adaptation_loss(self, x, edge_index, adapt_idx):
        ...
        _, z_causal, _, causal_logits, _ = self.encode_representation(
            x,
            edge_index,
            training=False,
        )
        ...
        with torch.no_grad():
            ...
            _, z_aug, _, causal_aug, _ = self.encode_representation(
                x_aug,
                edge_aug,
                training=False,
            )
    ...
    def compute_losses(self, data, criterion, args, update_state=False):
        ...
        contexts = self.sample_frontdoor_contexts(
            self.get_frontdoor_contexts(spurious_tr, pseudo_env_tr),
            training=True,
        )
