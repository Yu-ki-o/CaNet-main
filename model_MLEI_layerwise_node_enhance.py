...
from model_canet_layerwise_node_enhance import LayerwiseNodeEnhancementMixin
...
class MLEI(LayerwiseNodeEnhancementMixin, nn.Module):
    """
    ...
    def forward(self, x, adj, idx=None, training=False, return_details=False, pred_mode=None):
        ...
        local_env_list = []
        layerwise_gate_loss = h0.new_zeros(())
        layerwise_gate_mean = h0.new_zeros(())
        ...
        for layer_id, conv in enumerate(self.convs):
            ...
            if should_enhance:
                fused_repr, edge_gate = self.layerwise_node_enhance(
                    fused_repr,
                    adj,
        ...
        if self.use_layerwise_local_igm:
            if self.layerwise_final_edge_fuse:
                ...
                fused_repr = self.node_edge_norm(fused_repr)
        ...
        self._last_enhanced_repr = fused_repr
        ...
        if return_details:
            return {
                ...
                "pre_enhance_repr": h_pre_enhance,
                "enhanced_repr": fused_repr,
                ...
                "layerwise_gate_layers": self._last_layerwise_gate_layers,
            }
    ...
    def loss_compute(self, d, criterion, args):
        ...
        zero = local_pred_loss.new_zeros(())
        loss_layerwise_gate = (
        ...
        loss_enhance_sem = self.compute_enhance_semantic_loss(
            details["enhanced_repr"],
            details["pre_enhance_repr"],
            train_idx,
        )
        total_loss = (
            loss_local
            + args.lamda * loss_global
