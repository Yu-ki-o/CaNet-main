...
from model_stable import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--edge_feat_mode', type=str, default='mul',
                        choices=['mul', 'diff', 'degree', 'mul_diff', 'concat', 'concat_diff',
                                 'anchor_orth', 'stable_qk', 'mul_degree', 'diff_degree', 'mul_diff_degree'],
                        help=(
                            'edge feature used by the edge reliability MLP: '
                            'mul=h_src*h_dst, diff=|h_src-h_dst|, concat=[h_src,h_dst], '
                            'anchor_orth=anchor + orthogonal decomposition, stable_qk=stable anchor query-key attention, '
    ...
    parser.add_argument('--stable_qk_lambda', type=float, default=0.2,
                        help='dynamic correction ratio for stable_qk routing; should stay below 0.5')
    parser.add_argument('--stable_qk_env_margin', type=float, default=0.0,
                        help='margin before dynamic attention excess is routed to the environment context in stable_qk mode')
    parser.add_argument('--stable_qk_env_scale', type=float, default=1.0,
                        help='scale for the environment/context excess attention in stable_qk mode')
    parser.add_argument('--edge_context_alpha', type=float, default=0.0,
...
print(
    ...
    f"stable_qk: lambda={args.stable_qk_lambda}, env_margin={args.stable_qk_env_margin}, env_scale={args.stable_qk_env_scale}, "
    f"edge_ctx_alpha={args.edge_context_alpha}, ctx_final={args.edge_context_include_final} | "
