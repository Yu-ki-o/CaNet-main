...
class CIWModule(nn.Module):
    ...
    def forward(self, x_raw, h_init, idx=None):
        ...
        if self.training and idx is not None:
            ...
            # 强制计算加权方差并进行 Z-Score 归一化，锁死网络把特征全变0的后路！
            var_w = torch.sum(w_norm_train * (h_centered ** 2), dim=0, keepdim=True) / (N_train - 1 + 1e-8)
            std_w = torch.sqrt(var_w + 1e-5)
            h_norm = h_centered / std_w  # 此时每个维度的方差恒定为 1
            
            # 6. 使用标准化后的特征计算【加权相关系数矩阵】
            cov_w = torch.matmul(h_norm.T, w_norm_train * h_norm) / (N_train - 1 + 1e-8)
            
            # 7. 施加差异化掩码
            masked_cov = S * cov_w
            
            # 8. 【尺度修复】：计算剔除对角线后的 Mean Square Error，防止 Loss 爆炸
            D = cov_w.shape[0]
    ...
    def forward(self, x, adj, e, weights=None):
        ...
        if self.backbone_type == 'gcn':
            if not self.variant:
                ...
                adj_sparse = torch.sparse_coo_tensor(adj, torch.ones(adj.shape[1]).to(self.device), size=(x.shape[0],x.shape[0])).to(self.device)
            ...
            for k in range(self.K):
                edge_e_k = edge_e[k, :] # [E]
                e_expsum_k = self.specialspmm(adj, edge_e_k, torch.Size([N, N]), torch.ones(N, 1).to(self.device)) + eps
...
class CaNet(nn.Module):
    ...
    def forward(self, x, adj, idx=None, training=False):
        ...
        # 3. 特征过滤/提纯：保留因果强度高的特征，过滤环境噪音 (Ca 越小，特征越被削弱)
    ...
    def loss_compute(self, dataset, criterion, args):
        ...
        # 3. 【调试期保守参数】：防止辅任务抢夺梯度导致主任务坍塌
        lamda_reg = args.lamda  # 默认的 CaNet 环境正则化系数
        lamda_ciw = getattr(args, 'lamda_ciw', 1e-4) # 降到 0.1，先让主网络站稳脚跟
        lamda_l1 = getattr(args, 'lamda_l1', 0.5)  # 暂时归 0，防止网络把因果向量直接清零作弊
        ...
        if self.training and np.random.rand() < 0.05:
            print(f"[探照灯] 监督Loss: {sup_loss.item():.4f} | 环境正则Loss: {reg_loss.item():.4f} | CIW解耦Loss: {ciw_loss.item():.4f}")
