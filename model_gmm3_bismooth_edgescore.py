...
class GraphFrontDoorDAG(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # Edge-score source. In the original model, edge_gate is produced by
        # semantic endpoint features such as h_src * h_dst (mul). For the
        # local bi-smoothing edge-score variant, edge_gate is estimated from
        # randomized edge/node deletion: an edge receives a high score if the
        # destination node's mediator representation/prediction is more stable
        # when this edge is kept than when it is dropped.
        self.edge_score_source = getattr(args, 'edge_score_source', 'semantic')
        if self.edge_score_source not in ('semantic', 'bismooth', 'hybrid'):
            self.edge_score_source = 'semantic'
        self.bismooth_edge_score_samples = max(1, int(getattr(args, 'bismooth_edge_score_samples', self.bismooth_samples)))
        self.bismooth_score_temp = max(1e-3, float(getattr(args, 'bismooth_score_temp', 1.0)))
        self.bismooth_score_label_weight = max(0.0, float(getattr(args, 'bismooth_score_label_weight', 0.0)))
        self.bismooth_edge_mix = min(max(float(getattr(args, 'bismooth_edge_mix', 1.0)), 0.0), 1.0)
        self.bismooth_score_min_count = max(1, int(getattr(args, 'bismooth_score_min_count', 1)))
        self.bismooth_edge_score_ema = min(max(float(getattr(args, 'bismooth_edge_score_ema', 0.0)), 0.0), 1.0)
        self.bismooth_score_valid_edges = 0.0
        self.bismooth_score_mean = 0.5
        self._cached_bismooth_edge_gate = None
    ...
    def compute_edge_summaries(self, h, edge_index, training=False, external_edge_gate=None):
        """
        Build useful and low-relevance neighbor summaries.

        edge_score_source controls g_uv:
        - semantic: original endpoint-feature gate, e.g. mul/diff/degree.
        - bismooth: gate estimated from node-aware bi-smoothing stability.
        - hybrid: convex combination of semantic and bismooth gates.

        This makes the random-smoothing score the actual edge gate instead of
        merely adding a consistency regularizer on top of the old mul gate.
        """
        ...
        semantic_gate = None
        if self.edge_score_source in ('semantic', 'hybrid') or external_edge_gate is None:
            ...
            semantic_gate = torch.sigmoid(edge_logits)

        bismooth_gate = external_edge_gate
        if bismooth_gate is None and self._cached_bismooth_edge_gate is not None:
            if self._cached_bismooth_edge_gate.numel() == edge_index.size(1):
                bismooth_gate = self._cached_bismooth_edge_gate.to(device=h.device, dtype=h.dtype)
        if bismooth_gate is not None:
            bismooth_gate = bismooth_gate.to(device=h.device, dtype=h.dtype).clamp(0.0, 1.0)

        if self.edge_score_source == 'bismooth':
            if bismooth_gate is None:
                edge_gate = h.new_full((edge_index.size(1),), 0.5)
            else:
                edge_gate = bismooth_gate.detach()
        elif self.edge_score_source == 'hybrid':
            if semantic_gate is None:
                semantic_gate = h.new_full((edge_index.size(1),), 0.5)
            if bismooth_gate is None:
                edge_gate = semantic_gate
            else:
                mix = self.bismooth_edge_mix
                edge_gate = (1.0 - mix) * semantic_gate + mix * bismooth_gate.detach()
        else:
            edge_gate = semantic_gate
    ...
    def encode_backbone_only(self, x, edge_index, training=False):
    ...
    def encode_representation(self, x, edge_index, training=False, external_edge_gate=None):
        h = self.encode_backbone_only(x, edge_index, training=training)
        ...
        edge_summary, noise_summary, _ = self.compute_edge_summaries(
            ...
            edge_index,
            training=training,
            external_edge_gate=external_edge_gate,
        )
    ...
    def forward(self, x, edge_index, training=False, external_edge_gate=None):
        (
            z,
            edge_summary,
            dag_vars,
    ...
    def apply_state_update(self, state_payload):
        ...
        gate = state_payload.get('bismooth_edge_gate')
        if gate is not None:
            self._cached_bismooth_edge_gate = gate.detach().to(device=self.A_feat.device)
        if 'bismooth_edge_valid_ratio' in state_payload:
            self.bismooth_score_valid_edges = float(state_payload['bismooth_edge_valid_ratio'].detach().cpu())
        if 'bismooth_edge_gate_mean' in state_payload:
            self.bismooth_score_mean = float(state_payload['bismooth_edge_gate_mean'].detach().cpu())

    def sample_bismooth_edge_index(self, edge_index, num_nodes, train_idx=None, return_keep_mask=False):
        """
        ...
        if edge_index is None or edge_index.numel() == 0:
            ...
            if return_keep_mask:
                keep_mask = torch.ones(0, device=edge_index.device, dtype=torch.bool)
                return edge_index, valid_nodes, keep_mask
        ...
        if keep.sum() == 0:
            ...
            keep_mask = torch.ones(src.size(0), device=device, dtype=torch.bool)
            if return_keep_mask:
                return smooth_edge_index, valid_nodes, keep_mask
        ...
        if return_keep_mask:
            return smooth_edge_index, valid_nodes, keep
        return smooth_edge_index, valid_nodes

    @torch.no_grad()
    def estimate_bismooth_edge_gate(self, x, edge_index, train_idx, y, criterion, args):
        """Estimate edge gates from node-aware bi-smoothing stability.

        For each original directed edge e=(u->v), we repeatedly sample a
        bi-smoothed graph and compare the destination node v's stability when
        e is kept versus when e is dropped. A high score means that keeping the
        edge tends to preserve the clean mediator representation and/or improve
        the supervised prediction of v, so the edge is treated as invariant.
        """
        num_edges = edge_index.size(1)
        if (
            self.edge_score_source not in ('bismooth', 'hybrid')
            or num_edges == 0
            or train_idx is None
            or train_idx.numel() == 0
        ):
            return None, x.new_tensor(0.0), x.new_tensor(0.5)

        device = edge_index.device
        src, dst = edge_index
        num_nodes = x.size(0)

        # Clean mediator proxy before edge-gated fusion. This avoids circularly
        # using the old semantic edge gate to define the bi-smoothing score.
        h_clean = self.encode_backbone_only(x, edge_index, training=False)
        causal_score, pollution_score, mediator_gate, dag_total = self.get_causal_effect_and_mask()
        gate = mediator_gate.to(device=h_clean.device, dtype=h_clean.dtype).unsqueeze(0)
        z_clean = h_clean * gate
        logits_clean = self.classifier(z_clean)

        keep_sum = x.new_zeros(num_edges)
        keep_cnt = x.new_zeros(num_edges)
        drop_sum = x.new_zeros(num_edges)
        drop_cnt = x.new_zeros(num_edges)

        samples = max(1, int(self.bismooth_edge_score_samples))
        for _ in range(samples):
            smooth_edge_index, valid_nodes, keep_mask = self.sample_bismooth_edge_index(
                edge_index,
                num_nodes,
                train_idx=train_idx,
            ...
            h_smooth = self.encode_backbone_only(x, smooth_edge_index, training=False)
            z_smooth = h_smooth * gate
            logits_smooth = self.classifier(z_smooth)

            idx = train_idx[train_valid]
            cos = (
                F.normalize(z_smooth[idx], dim=1)
                * F.normalize(z_clean[idx], dim=1)
            ).sum(dim=1)
            stability = cos
            if self.bismooth_score_label_weight > 0.0:
                per_node_loss = self.compute_supervised_loss(
                    logits_smooth[idx],
                    y[idx],
                    criterion,
                    args,
                )
                # Normalize within the sampled valid nodes so CE scale does not
                # dominate cosine stability. Larger stability is better.
                loss_norm = (per_node_loss - per_node_loss.mean()) / per_node_loss.std(unbiased=False).clamp_min(1e-6)
                stability = stability - self.bismooth_score_label_weight * loss_norm

            node_stability = x.new_zeros(num_nodes)
            node_observed = torch.zeros(num_nodes, device=device, dtype=torch.bool)
            node_stability[idx] = stability
            node_observed[idx] = True
            edge_observed = node_observed[dst]
            edge_value = node_stability[dst]

            kept = keep_mask & edge_observed
            dropped = (~keep_mask) & edge_observed
            keep_sum = keep_sum + kept.to(x.dtype) * edge_value
            keep_cnt = keep_cnt + kept.to(x.dtype)
            drop_sum = drop_sum + dropped.to(x.dtype) * edge_value
            drop_cnt = drop_cnt + dropped.to(x.dtype)

        valid = (keep_cnt >= self.bismooth_score_min_count) & (drop_cnt >= self.bismooth_score_min_count)
        gate_out = x.new_full((num_edges,), 0.5)
        score = x.new_zeros(num_edges)
        if valid.any():
            keep_avg = keep_sum[valid] / keep_cnt[valid].clamp_min(1.0)
            drop_avg = drop_sum[valid] / drop_cnt[valid].clamp_min(1.0)
            raw = keep_avg - drop_avg
            raw = (raw - raw.mean()) / raw.std(unbiased=False).clamp_min(1e-6)
            gate_valid = torch.sigmoid(raw / self.bismooth_score_temp)
            gate_out[valid] = gate_valid
            score[valid] = raw

        # Optional EMA stabilizes evaluation and prevents the edge gate from
        # changing too abruptly across epochs.
        if self._cached_bismooth_edge_gate is not None and self._cached_bismooth_edge_gate.numel() == num_edges:
            if self.bismooth_edge_score_ema > 0.0:
                old = self._cached_bismooth_edge_gate.to(device=device, dtype=gate_out.dtype)
                gate_out = self.bismooth_edge_score_ema * old + (1.0 - self.bismooth_edge_score_ema) * gate_out
        valid_ratio = valid.float().mean() if num_edges > 0 else x.new_tensor(0.0)
        mean_gate = gate_out.mean() if num_edges > 0 else x.new_tensor(0.5)
        return gate_out.detach(), valid_ratio.detach(), mean_gate.detach()
    ...
    def compute_losses(self, data, criterion, args, update_state=False):
        ...
        bismooth_edge_gate, bismooth_edge_valid_ratio, bismooth_edge_gate_mean = self.estimate_bismooth_edge_gate(
            x,
            edge_index,
            train_idx,
            y,
        ...
        (
            ...
            edge_index,
            training=True,
            external_edge_gate=bismooth_edge_gate,
        )
        ...
        if update_state:
            state_payload = {
                ...
                'bismooth_edge_gate': bismooth_edge_gate.detach() if bismooth_edge_gate is not None else None,
                'bismooth_edge_valid_ratio': bismooth_edge_valid_ratio.detach(),
                'bismooth_edge_gate_mean': bismooth_edge_gate_mean.detach(),
            }
