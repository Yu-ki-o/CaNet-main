...
class GraphFrontDoorDAG(nn.Module):
    """
    Front-door graph model with CIPT-style causal/spurious adapters.

    Main idea:
    1) Encode nodes with the GNN backbone.
    2) Build a NodeIGM-style edge semantic summary from endpoint hidden states.
    3) Split node representations with two lightweight CIPT adapters.
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # CGRL-aligned Energy-Based Reconstruction (EBR) applied to the causal
        # stable feature z_mediator produced by the causal adapter.  This adds a
        # reverse constraint H_c -> Q_phi(G | H_c), encouraging the causal branch
        # to preserve stable local graph structure instead of only fitting labels.
        ...
        self.ebr_nce_weight = max(0.0, float(getattr(args, 'ebr_nce_weight', 1.0)))
        self.ebr_kl_weight = max(0.0, float(getattr(args, 'ebr_kl_weight', 0.1)))
        self.ebr_kl_direction = getattr(args, 'ebr_kl_direction', 'p_to_q')
        if self.ebr_kl_direction not in ('p_to_q', 'q_to_p'):
            self.ebr_kl_direction = 'p_to_q'
    ...
    def encode_representation(self, x, edge_index, training=False):
        ...
        # z_spurious = self.causal_norm(self.spurious_adapter(z))
        ...
        if self.use_layerwise_spurious_contexts and layerwise_states:
            ...
            layerwise_spurious = self.spurious_norm(
                layerwise_stack + self.spurious_adapter(layerwise_stack)
            )
    ...
    def compute_ebr_loss(self, h_causal, edge_index, training=False):
        """
        CGRL-aligned Energy-Based Reconstruction on the causal stable features.

        h_causal corresponds to H_c after the causal adapter.  The loss builds
        Q_phi(G | H_c) with an EBM edge energy, normalizes reconstructed edge
        probabilities over observed incoming neighbours, and aligns them to a
        stable graph prior.  A pairwise NCE term is also used so observed edges
        have lower energy than sampled non-edges.
        """
        ...
        # Q_phi(u, v | H_c): reconstructed energy posterior over observed
        # incoming neighbours.
        ...
        # P_theta(G): stable graph prior over observed neighbours.  We use a
        # uniform observed-neighbour prior here to avoid a moving self-target;
        # the edge-score normalization is already used in the forward enhancer.
        ...
        if self.ebr_kl_direction == 'q_to_p':
            kl_loss = (q_edge * (log_q_edge - log_p_edge.detach())).sum() / max(1, int(active_dst))
        else:
            kl_loss = (p_edge * (log_p_edge - log_q_edge)).sum() / max(1, int(active_dst))
    ...
    def compute_losses(self, data, criterion, args, update_state=False):
        ...
        # CGRL-style EBR is applied on the causal stable feature from the causal adapter.
        loss_ebr = self.compute_ebr_loss(z_mediator_all, edge_index, training=True)
