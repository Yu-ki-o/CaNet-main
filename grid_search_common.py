...
def write_topk_summary(results, summary_path, dataset, stage, hp_keys):
    ...
    with open(summary_path, "w") as summary_file:
        summary_file.write(
            f"Top {len(top_results)} experiments on {dataset} ({stage}) "
...
def run_grid_search(dataset, search_stage, base_cmd, stage_grids, search_root="search_logs"):
    ...
    log_dir = os.path.join(search_root, dataset, search_stage)
    os.makedirs(log_dir, exist_ok=True)

    all_results = []

    print(f"Start {search_stage} grid search on {dataset}, total runs: {len(combinations)}")
    print("-" * 80)
    ...
    for idx, combination in enumerate(combinations, start=1):
        ...
        result_name_parts = [dataset, search_stage]
        ...
        all_results_csv = os.path.join(log_dir, f"{dataset}_{search_stage}_all_results.csv")
        all_results_json = os.path.join(log_dir, f"{dataset}_{search_stage}_all_results.json")
        topk_summary = os.path.join(log_dir, f"{dataset}_{search_stage}_top{TOP_K}.txt")
