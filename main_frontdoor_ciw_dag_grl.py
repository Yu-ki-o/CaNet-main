...
from model_frontdoor_ciw_dag_grl import GraphFrontDoorCIWDAGGRL
...
def add_grl_args(parser):
    """Arguments for EGOG-style gradient reversal disentanglement."""
    parser.add_argument('--use_grl', action='store_true',
                        help='enable GRL losses: mediator -> no env info, spurious -> no label info')
    parser.add_argument('--lambda_grl_env', type=float, default=0.0,
                        help='weight for GRL environment adversary on mediator/causal representation')
    parser.add_argument('--lambda_grl_y', type=float, default=0.0,
                        help='weight for GRL label adversary on spurious representation')
    parser.add_argument('--grl_env_coeff', type=float, default=1.0,
                        help='gradient reversal coefficient for mediator-to-environment adversary')
    parser.add_argument('--grl_y_coeff', type=float, default=1.0,
                        help='gradient reversal coefficient for spurious-to-label adversary')
    parser.add_argument('--grl_replace_uniform', action='store_true',
                        help='replace original uniform env/spurious losses with GRL losses')
    parser.add_argument('--disable_grl_pseudo_env', action='store_false', dest='grl_use_pseudo_env',
                        help='disable pseudo-env targets for GRL when true env labels are unavailable')
    parser.set_defaults(grl_use_pseudo_env=True)
...
parser = argparse.ArgumentParser(description='Graph Front-Door CIW-DAG-GRL Training Pipeline')
...
model = GraphFrontDoorCIWDAGGRL(d, c, args, device).to(device)
...
log_dir = os.path.join('.', 'runs', args.dataset, 'frontdoor_ciw_dag_grl', run_name)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if 'loss_grl_env' in losses:
            writer.add_scalar('Loss/GRLEnv', (getattr(model, 'lambda_grl_env', 0.0) * losses['loss_grl_env']).item(), global_step)
        if 'loss_grl_y' in losses:
            writer.add_scalar('Loss/GRLY', (getattr(model, 'lambda_grl_y', 0.0) * losses['loss_grl_y']).item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"GRLEnv: {(getattr(model, 'lambda_grl_env', 0.0) * losses.get('loss_grl_env', losses['loss_env_med'] * 0)).item():.4f}, "
                f"GRLY: {(getattr(model, 'lambda_grl_y', 0.0) * losses.get('loss_grl_y', losses['loss_env_med'] * 0)).item():.4f}, "
