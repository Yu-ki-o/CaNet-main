...
from model_fd_two_layer_fdedge import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--edge_feat_mode', type=str, default='mul',
                        ...
                        help=(
                            ...
                            'anchor_orth=anchor + orthogonal decomposition, '
                            'fd_edge=front-door stable edge routing, '
    ...
    parser.add_argument('--anchor_orth_lambda', type=float, default=0.2,
                        help='dynamic correction ratio for anchor_orth invariant gate; should stay below 0.5')
    parser.add_argument('--anchor_orth_perp_scale', type=float, default=1.0,
                        help='scale of the orthogonal residual penalty/context gate in anchor_orth mode')
    parser.add_argument('--lambda_dynamic', type=float, default=0.2,
                        help='dynamic correction ratio for fd_edge routing; should stay below 0.5')
    parser.add_argument('--stable_qk_lambda', type=float, default=None,
                        help=argparse.SUPPRESS)
    parser.add_argument('--stable_qk_env_margin', type=float, default=0.0,
                        help='margin before dynamic attention excess is routed to the environment context in fd_edge mode')
    parser.add_argument('--stable_qk_env_scale', type=float, default=1.0,
                        help='scale for the environment/context excess attention in fd_edge mode')
    parser.add_argument('--env_alpha', type=float, default=0.0,
                        help='add environment edge summaries to the spurious/context branch with this residual strength')
    parser.add_argument('--edge_context_alpha', type=float, default=None,
                        help=argparse.SUPPRESS)
    parser.add_argument('--disable_edge_context_include_final', action='store_false',
                        dest='edge_context_include_final',
                        help='do not add the final environment edge summary into the edge context accumulator')
    parser.set_defaults(edge_context_include_final=True)
...
if args.stable_qk_lambda is not None:
    args.lambda_dynamic = args.stable_qk_lambda
if args.edge_context_alpha is not None:
    args.env_alpha = args.edge_context_alpha
print(args)
...
print(
    ...
    f"anchor_orth: lambda={args.anchor_orth_lambda}, perp_scale={args.anchor_orth_perp_scale}, "
    f"fd_edge: lambda_dynamic={args.lambda_dynamic}, env_margin={args.stable_qk_env_margin}, env_scale={args.stable_qk_env_scale}, "
    f"env_alpha={args.env_alpha}, ctx_final={args.edge_context_include_final} | "
