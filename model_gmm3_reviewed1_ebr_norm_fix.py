...
class GraphFrontDoorDAG(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # CGRL/GAT-style neighbour normalization for relation-aware routing.
        # When enabled, edge scores are normalized over each destination node's
        # incoming neighbours: alpha_uv = softmax_u(score_uv).  The normalized
        # alpha can still be multiplied by the GCN degree coefficient, matching
        # the paper's combination of edge/attention weights and C_uv.
    ...
    def compute_edge_summaries(self, h, edge_index, training=False):
        """
        Build both useful and low-relevance neighbour summaries.

        With --use_edge_score_norm enabled, edge logits are normalized over
        each destination node's incoming neighbours:

            alpha_uv = softmax_{u in N(v)}(score_uv / tau).

        The useful branch uses alpha_uv.  The low-relevance branch uses the
        reverse normalized score softmax(-score_uv / tau), so it remains a
        proper neighbour distribution instead of the unnormalized (1-alpha).
        If --edge_norm_with_degree is kept enabled, both branches are also
        multiplied by the GCN degree coefficient deg(u)^-1/2 deg(v)^-1/2.
        """
        ...
        if self.use_edge_score_norm:
            # Useful neighbours compete only within the same destination node.
            edge_gate = softmax(edge_logits / self.edge_norm_temp, dst, num_nodes=num_nodes)
            # Low-score neighbours are normalized separately for the optional
            # denoising branch; this avoids treating 1-alpha as a probability.
            low_edge_gate = softmax(-edge_logits / self.edge_norm_temp, dst, num_nodes=num_nodes)
            if self.edge_norm_with_degree:
                useful_weight = norm * edge_gate
                noise_weight = norm * low_edge_gate
            else:
                useful_weight = edge_gate
                noise_weight = low_edge_gate
        else:
            edge_gate = torch.sigmoid(edge_logits)
            useful_weight = norm * edge_gate
            noise_weight = norm * (1.0 - edge_gate)
    ...
    def _normalize_edge_distribution(self, weights, dst, num_nodes):
        """Normalize positive weights over each destination node."""
        weights = weights.clamp_min(1e-12)
        norm = weights.new_zeros(num_nodes)
        norm.index_add_(0, dst, weights)
        return weights / norm[dst].clamp_min(1e-12)
    ...
    def compute_energy_reconstruction_loss(self, h, edge_index, final_edge_gate=None, training=False):
        ...
        Important implementation detail: in the code, P_theta(G) is the graph
        prior/teacher distribution from the encoder edge scorer, while
        Q_phi(G | h) is the reconstructed posterior from the bilinear energy.
        Since P is the target we want the reconstruction to match, the default
        optimization uses KL(P || Q), implemented as cross-entropy from the
        detached prior to the reconstructed log-posterior.  This is numerically
        more stable than pushing gradients through Q in KL(Q || P), which can
        grow when the task loss sharpens Q away from a smooth graph prior.

        The energy score is still aligned with the paper:
            E(u, v) = - h_v^T W h_u,
        and the returned logit is -E(u, v). Larger logits therefore mean lower
        energy and a more likely reconstructed edge.
        """
        zero = h.new_zeros(())
        ...
        # Q_phi(u, v | h): reconstructed energy posterior over observed incoming neighbours.
        ...
        # P_theta(G): target graph prior. If the encoder edge prior is available, it is
        # already a neighbour-normalized distribution; after edge subsampling we simply
        # renormalize the remaining mass per destination. Otherwise use a uniform prior
        # over observed incoming neighbours.
        if self.ebr_use_edge_prior and pos_prior_gate is not None:
            p_edge = self._normalize_edge_distribution(pos_prior_gate.to(dtype=h.dtype), pos_dst, num_nodes)
        ...
        if self.ebr_kl_direction == 'q_to_p':
            # Strict paper-form KL(Q_phi(G|H) || P_theta(G)). Kept as an option.
            kl_loss = (q_edge * (log_q_edge - log_p_edge.detach())).sum() / max(1, int(active_dst))
        else:
            # Stable reconstruction form: KL(P_theta(G) || Q_phi(G|H)).
            kl_loss = (p_edge * (log_p_edge - log_q_edge)).sum() / max(1, int(active_dst))
        self._last_ebr_kl_loss = kl_loss.detach()

        # Pairwise sampled reconstruction: positives should have lower energy than sampled non-edges.
        # This avoids the instability of making all observed neighbours compete as negatives of
        # each other when a destination has many true neighbours.
        nce_loss = zero
        bce_loss = zero
