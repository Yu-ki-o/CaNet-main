...
from model_frontdoor_ciw_dag_edge import GraphFrontDoorCIWDAGEdge
...
def add_edge_dag_args(parser):
    """Arguments for injecting learned edge factors into the DAG variable space."""
    parser.add_argument('--disable_edge_dag', action='store_false', dest='use_edge_dag',
                        help='disable DAG-internal learned edge factor injection')
    parser.set_defaults(use_edge_dag=True)
    parser.add_argument('--edge_hidden_dim', type=int, default=64,
                        help='hidden dimension expected for z_all in the edge encoder; usually equals hidden_channels')
    parser.add_argument('--edge_factor_scale', type=float, default=None,
                        help='scale for projected edge factors; default uses sqrt(non_label_var_dim / edge_var_dim)')
    parser.add_argument('--edge_inject_dropout', type=float, default=0.0,
                        help='dropout used inside the edge message MLP')
    parser.add_argument('--lambda_edge_dag', type=float, default=0.05,
                        help='auxiliary weight for edge-injected DAG label readout')
    parser.add_argument('--lambda_edge_gate_balance', type=float, default=0.0,
                        help='optional regularizer keeping edge/original latent mixing from collapsing too early')
    parser.add_argument('--edge_readout_blend', type=float, default=0.2,
                        help='maximum contribution of edge-injected DAG readout to the primary logits')
...
parser = argparse.ArgumentParser(description='Graph Front-Door CIW-DAG Training Pipeline with DAG-internal Edge Factors')
parser_add_main_args(parser)
add_frontdoor_dag_args(parser)
add_ciw_dag_args(parser)
add_edge_dag_args(parser)
...
model = GraphFrontDoorCIWDAGEdge(d, c, args, device).to(device)
...
print(
    ...
    f"edge DAG: {args.use_edge_dag} (lambda={args.lambda_edge_dag}, blend={args.edge_readout_blend})"
)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if 'loss_edge_dag' in losses:
            writer.add_scalar('Loss/EdgeDAG', (getattr(model, 'lambda_edge_dag', 0.0) * losses['loss_edge_dag']).item(), global_step)
        if 'loss_edge_gate_balance' in losses:
            writer.add_scalar('Loss/EdgeGateBalance', (getattr(model, 'lambda_edge_gate_balance', 0.0) * losses['loss_edge_gate_balance']).item(), global_step)
        if 'edge_gate_mean' in losses:
            writer.add_scalar('Edge/GateMean', losses['edge_gate_mean'].item(), global_step)
        if 'edge_mix_mean' in losses:
            writer.add_scalar('Edge/MixMean', losses['edge_mix_mean'].item(), global_step)
        if 'node_edge_readout_edge_mean' in losses:
            writer.add_scalar('Edge/ReadoutEdgeWeightRaw', losses['node_edge_readout_edge_mean'].item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"EdgeGate: {losses.get('edge_gate_mean', torch.tensor(0.)).item():.3f}, "
                f"EdgeMix: {losses.get('edge_mix_mean', torch.tensor(0.)).item():.3f}, "
