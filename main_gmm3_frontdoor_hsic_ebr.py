...
from model_gmm3_frontdoor_hsic_ebr import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--ebr_kl_weight', type=float, default=1.0,
                        help='weight of KL(Q_phi(G|M)||P_theta(G)) inside EBR')
    parser.add_argument('--lambda_node_post', type=float, default=0.0,
                        help='weight of mediator semantic posterior consistency against detached enhanced features')
    parser.add_argument('--node_post_dim', type=int, default=32,
                        help='low-dimensional posterior size for mediator semantic consistency')
    parser.add_argument('--node_post_temperature', type=float, default=1.0,
                        help='temperature for mediator semantic posterior consistency')
    parser.add_argument('--node_post_entropy_weight', type=float, default=0.0,
                        help='optional entropy-floor penalty weight for mediator semantic posterior')
    parser.add_argument('--node_post_min_entropy', type=float, default=0.2,
                        help='minimum normalized entropy target for mediator semantic posterior')
...
print(
    ...
    f"EBR: lambda={args.lambda_ebr}, temp={args.ebr_temperature}, prior={args.ebr_prior}, "
    f"kl={args.ebr_kl_weight}/{args.ebr_kl_direction}, nce={args.ebr_nce_weight}, "
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Loss/NodePost', (model.lambda_node_post * losses['loss_node_post']).item(), global_step)
        writer.add_scalar('Loss/NodePostRaw', losses['loss_node_post'].item(), global_step)
        writer.add_scalar('Graph/NodePostEntropy', losses['node_post_entropy'].item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"NodePost: {(model.lambda_node_post * losses['loss_node_post']).item():.4f}, "
                f"NodeEnt: {losses['node_post_entropy'].item():.3f}, "
