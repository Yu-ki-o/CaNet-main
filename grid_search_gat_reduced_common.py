...
def write_topk_summary(results, summary_path, dataset, stage, hp_keys):
    ...
    with open(summary_path, "w") as summary_file:
        summary_file.write(f"Top {len(top_results)} experiments on {dataset} ({stage}, GAT reduced)\n")
...
def run_grid(dataset, stage, base_cmd, param_grid):
    keys = list(param_grid.keys())
    combinations = list(itertools.product(*param_grid.values()))
    log_dir = f"search_logs/{dataset}/{stage}_gat_reduced"
    os.makedirs(log_dir, exist_ok=True)
    all_results = []

    print(f"Start {stage} reduced GAT grid search on {dataset}, total runs: {len(combinations)}")
    for idx, combination in enumerate(combinations, start=1):
        current_params = dict(zip(keys, combination))
        cmd = base_cmd.copy()
        result_name_parts = [dataset, stage, "gat_reduced"]
        ...
        write_all_results_csv(all_results, os.path.join(log_dir, f"{dataset}_{stage}_gat_reduced_all_results.csv"))
        with open(os.path.join(log_dir, f"{dataset}_{stage}_gat_reduced_all_results.json"), "w") as json_file:
            json.dump(all_results, json_file, indent=2)
        write_topk_summary(
            all_results,
            os.path.join(log_dir, f"{dataset}_{stage}_gat_reduced_top{TOP_K}.txt"),
            dataset,
            stage,
            keys,
        )
