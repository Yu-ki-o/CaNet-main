...
class GraphFrontDoorDAG(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # Which endpoint interaction is used to score an edge.
        # - full: original [h_src, h_dst, h_src * h_dst, |h_src - h_dst|]
        # - mul: only the third part, h_src * h_dst
        # - diff: only the fourth part, |h_src - h_dst|
        # - mul_diff: only [h_src * h_dst, |h_src - h_dst|]
        # This lets us test whether the Arxiv gain comes from the interaction
        # term alone instead of the full 4d pair representation.
        self.edge_feat_mode = getattr(args, 'edge_feat_mode', 'full')
        valid_edge_feat_modes = {'full', 'mul', 'diff', 'mul_diff'}
        if self.edge_feat_mode not in valid_edge_feat_modes:
            raise ValueError(
                f"Unsupported edge_feat_mode='{self.edge_feat_mode}'. "
                f"Use one of {sorted(valid_edge_feat_modes)}."
            )
        if self.edge_feat_mode == 'full':
            edge_feat_dim = self.d * 4
        elif self.edge_feat_mode in ('mul', 'diff'):
            edge_feat_dim = self.d
        else:  # mul_diff
            edge_feat_dim = self.d * 2
    ...
    def build_edge_pair_features(self, h_src, h_dst):
        if self.edge_feat_mode == 'full':
            return torch.cat(
                [
                    h_src,
        ...
        raise RuntimeError(f"Unexpected edge_feat_mode='{self.edge_feat_mode}'")
    ...
    def compute_edge_semantic_summary(self, h, edge_index, training=False):
        ...
        src, dst = edge_index
        edge_feat = self.build_edge_pair_features(h[src], h[dst])
