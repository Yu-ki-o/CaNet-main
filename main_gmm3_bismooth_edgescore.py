...
from model_gmm3_bismooth_edgescore import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--edge_score_source', type=str, default='semantic', choices=['semantic', 'bismooth', 'hybrid'],
                        help='edge gate source: semantic endpoint features, bismooth stability, or hybrid')
    parser.add_argument('--bismooth_edge_score_samples', type=int, default=1,
                        help='number of bi-smoothing samples used to estimate edge stability gates')
    parser.add_argument('--bismooth_score_temp', type=float, default=1.0,
                        help='temperature for converting bismooth stability contribution to edge gates')
    parser.add_argument('--bismooth_score_label_weight', type=float, default=0.0,
                        help='optional supervised-loss weight in the bismooth edge stability score')
    parser.add_argument('--bismooth_edge_mix', type=float, default=1.0,
                        help='hybrid mode: weight of bismooth gate when mixing with semantic edge gate')
    parser.add_argument('--bismooth_score_min_count', type=int, default=1,
                        help='minimum kept/dropped observations required to trust a bismooth edge score')
    parser.add_argument('--bismooth_edge_score_ema', type=float, default=0.0,
                        help='EMA momentum for bismooth edge gates cached for evaluation')
...
def apply_cipt_schedule(model, base_lambdas, epoch, args):
    ...
    for name in ('lambda_fd', 'lambda_fd_aug', 'lambda_var', 'lambda_env', 'lambda_inv', 'lambda_bismooth', 'lambda_bismooth_cls'):
...
print(
    ...
    f"edge score source: {args.edge_score_source} "
    f"(score_samples={args.bismooth_edge_score_samples}, score_temp={args.bismooth_score_temp}, "
    f"label_w={args.bismooth_score_label_weight}, mix={args.bismooth_edge_mix}, "
    f"score_ema={args.bismooth_edge_score_ema}) | "
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Graph/BiSmoothEdgeValidRatio', losses['bismooth_edge_valid_ratio'].item(), global_step)
        writer.add_scalar('Graph/BiSmoothEdgeGateMean', losses['bismooth_edge_gate_mean'].item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"BiEdgeValid: {losses['bismooth_edge_valid_ratio'].item():.3f}, "
                f"BiEdgeGate: {losses['bismooth_edge_gate_mean'].item():.3f}, "
                f"LR: {current_lr:.6f}, "
