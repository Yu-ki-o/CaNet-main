...
from model_frontdoor_dag_core_edgefeat_pubmed_edgecontext import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--use_edge_spu_context', action='store_true',
                        help='fuse low-relevance edge structural-spurious summary into z_spurious before pseudo-env/GMM/front-door context sampling')
    parser.add_argument('--edge_spu_threshold', type=float, default=0.35,
                        help='edge_gate below this threshold is treated as structural spurious')
    parser.add_argument('--edge_spu_temp', type=float, default=8.0,
                        help='temperature for converting low edge_gate values to structural-spurious weights')
    parser.add_argument('--edge_spu_context_alpha', type=float, default=0.3,
                        help='residual strength for injecting edge structural-spurious summary into environment context')
    parser.add_argument('--edge_spu_msg_mode', type=str, default='residual', choices=['residual', 'neighbor'],
                        help="structural-spurious message: residual uses h_src-h_dst, neighbor uses h_src")
...
print(
    ...
    f"edge-spu ctx: {args.use_edge_spu_context} (thr={args.edge_spu_threshold}, alpha={args.edge_spu_context_alpha}) | "
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Graph/EdgeSpuContextNorm', losses['edge_spu_context_norm'].item(), global_step)
        writer.add_scalar('Graph/EnvContextNorm', losses['env_context_norm'].item(), global_step)
