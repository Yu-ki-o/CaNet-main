...
from torch_geometric.utils import add_self_loops, degree, remove_self_loops, softmax, negative_sampling
...
class GraphFrontDoorDAG(nn.Module):
    """
    Front-door graph model with explicit causal mediator and spurious context.

    Variable mapping used by this implementation:
    X: enhanced node representation z.
    M: causal stable mediator z_mediator produced by the causal adapter.
    U: spurious/environment representation z_spurious produced by the spurious adapter.

    The prediction path follows a neural front-door adjustment pattern:
    1) z -> M -> Y is the direct predictive path.
    2) z_spurious is not fed directly to Y; only sampled/averaged environment
       contexts enter P(Y | M, U') as adjustment variables.
    3) HSIC(M, U) discourages a direct statistical path between mediator and
       spurious variables.
    4) CGRL-style EBR regularizes M through KL(Q_phi(G | M) || P_theta(G)).
    """
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # CGRL/GAT-style neighbour normalization for relation-aware node enhancement.
        # Edge scores are normalized over each destination node's incoming neighbours:
        #   alpha_uv = softmax_{u in N(v)}(score_uv / tau).
        # The normalized alpha can still be multiplied by the GCN degree coefficient.
        self.use_edge_score_norm = bool(getattr(args, 'use_edge_score_norm', True))
        self.edge_norm_temp = max(1e-3, float(getattr(args, 'edge_norm_temp', 1.0)))
        self.edge_norm_with_degree = bool(getattr(args, 'edge_norm_with_degree', True))
        ...
        # CITA-style local weighting without materializing an augmented adjacency:
        # the selected edge_feat_mode is scored by an MLP, then blended with a
        # centre-neighbour similarity prior before per-destination normalization.
        self.use_cita_edge_similarity = bool(getattr(args, 'use_cita_edge_similarity', True))
        self.cita_edge_sim_weight = max(0.0, float(getattr(args, 'cita_edge_sim_weight', 1.0)))
        self.cita_edge_mlp_weight = max(0.0, float(getattr(args, 'cita_edge_mlp_weight', 1.0)))
        ...
        # CIPT-style decomposition: two residual adapters replace the DAG
        # mediator gate while preserving the rest of the front-door pipeline.
        # self.causal_adapter = nn.Linear(self.d, self.d)
        # self.spurious_adapter = nn.Linear(self.d, self.d)


        # ==========================================
        # 升级版：非线性残差瓶颈层 (已修复 Dropout 顺序问题)
        # ==========================================
        bottleneck_dim = max(16, self.d // 2)  
        # 提前获取 dropout 概率
        dropout_prob = float(getattr(args, 'dropout', 0.0))
        ...
        # choose the mediator in this CIPT-adapter variant.
        ...
        self.use_dag_mixer = getattr(args, 'use_dag_mixer', False)
        ...
        # Loss weights kept in the front-door model.  Redundant learned-DAG
        # regularizers are disabled by default; the causal graph is enforced by
        # the mediator/context prediction path, HSIC(M,U), and EBR(M).
        self.lambda_l1 = getattr(args, 'lambda_l1', 1e-5)
        self.lambda_dag = getattr(args, 'lambda_dag', 0.0)
        self.lambda_med = getattr(args, 'lambda_med', 0.5)
        self.lambda_spu = getattr(args, 'lambda_spu', 0.05)
        self.lambda_fd = getattr(args, 'lambda_fd', 0.5)
        self.lambda_fd_aug = getattr(args, 'lambda_fd_aug', 0.0)
        self.lambda_var = getattr(args, 'lambda_var', 0.0)
        self.lambda_ind = getattr(args, 'lambda_ind', 0.0)
        self.lambda_env = getattr(args, 'lambda_env', 0.05)
        self.lambda_inv = getattr(args, 'lambda_inv', 0.0)
        self.lambda_global_env = getattr(args, 'lambda_global_env', 0.0)
        self.lambda_hsic = max(0.0, float(getattr(args, 'lambda_hsic', 0.05)))
        self.hsic_max_samples = max(0, int(getattr(args, 'hsic_max_samples', 2048)))
        self.hsic_kernel = getattr(args, 'hsic_kernel', 'rbf')
        if self.hsic_kernel not in ('rbf', 'linear'):
            self.hsic_kernel = 'rbf'
        self.hsic_sigma = float(getattr(args, 'hsic_sigma', -1.0))

        # CGRL formula-(4)-style Energy-Based Reconstruction (EBR) applied to
        # the causal mediator M.  The implemented core term is
        # KL(Q_phi(G | M) || P_theta(G)), where Q is reconstructed from EBM
        # edge energies and P is a detached encoder graph prior.  NCE/BCE are
        # optional diagnostics/auxiliary terms and are disabled by default.
        ...
        self.ebr_neg_ratio = max(0.0, float(getattr(args, 'ebr_neg_ratio', 0.0)))
        self.ebr_max_edges = int(getattr(args, 'ebr_max_edges', 200000))
        self.ebr_bce_weight = max(0.0, float(getattr(args, 'ebr_bce_weight', 0.0)))
        self.ebr_nce_weight = max(0.0, float(getattr(args, 'ebr_nce_weight', 0.0)))
        # Edge-posterior KL weight: Q_phi(u,v | M) vs. the encoder graph prior.
        self.ebr_kl_weight = max(0.0, float(getattr(args, 'ebr_kl_weight', 1.0)))
        # Node/Gumbel posterior weight: Q_phi(G_s | M) vs. P_theta(G_s).
        # This is the missing CGRL EBR term from Eq. (4): the graph posterior
        # is approximated by the concatenation/sum of node Gumbel sampling and
        # edge EBM posterior terms.
        self.ebr_node_weight = max(0.0, float(getattr(args, 'ebr_node_weight', 1.0)))
        self.ebr_node_temperature = max(1e-3, float(getattr(args, 'ebr_node_temperature', self.ebr_temperature)))
        self.ebr_use_gumbel = bool(getattr(args, 'ebr_use_gumbel', True))
        self.ebr_gumbel_eps = max(1e-12, float(getattr(args, 'ebr_gumbel_eps', 1e-10)))
        self.ebr_kl_direction = getattr(args, 'ebr_kl_direction', 'q_to_p')
        if self.ebr_kl_direction not in ('p_to_q', 'q_to_p'):
            self.ebr_kl_direction = 'q_to_p'
        self.ebr_prior = getattr(args, 'ebr_prior', 'encoder')
        if self.ebr_prior not in ('encoder', 'uniform'):
            self.ebr_prior = 'encoder'
        ...
        self._last_ebr_nce_loss = None
        self._last_ebr_kl_loss = None
        self._last_ebr_node_loss = None
        self._last_ebr_edge_loss = None
        self._last_ebr_pos_score = None
        self._last_ebr_neg_score = None
        self._last_ebr_num_pos = 0
        self._last_ebr_num_neg = 0

        self.lambda_proto_compact = max(0.0, float(getattr(args, 'lambda_proto_compact', 0.0)))
        self.lambda_proto_margin = max(0.0, float(getattr(args, 'lambda_proto_margin', 0.0)))
        self.proto_margin = max(0.0, float(getattr(args, 'proto_margin', 1.0)))
        self.proto_normalize = bool(getattr(args, 'proto_normalize', True))

        # Explicit saved spurious-environment context bank.  The GMM mean/var
        # buffers below are used for sampling, while this bank stores the
        # normalized environment prototypes from the spurious adapter branch for
        # inspection and for later front-door context reuse.
        self.save_spu_env_contexts = bool(getattr(args, 'save_spu_env_contexts', True))
        ...
        self.lambda_dag_label = getattr(args, 'lambda_dag_label', 0.0)
        ...
        self.register_buffer('saved_spu_env_contexts', torch.zeros(self.num_envs, self.d))
        self.register_buffer('saved_spu_env_context_valid', torch.zeros(self.num_envs, dtype=torch.bool))
    ...
    def reset_parameters(self):
        ...
        self.saved_spu_env_contexts.zero_()
        self.saved_spu_env_context_valid.zero_()
    ...
    def compute_edge_summaries(self, h, edge_index, training=False):
        """
        CITA-style relation-aware neighbour enhancement/weakening.

        useful_summary_v = sum_u norm_uv * alpha_uv^+ * h_u
        noise_summary_v  = sum_u norm_uv * alpha_uv^- * h_u

        Instead of constructing CITA's augmented adjacency, each observed edge is
        scored directly from the selected edge_feat_mode and a cosine-similarity
        prior between centre/neighbour states.  High-score neighbours enhance the
        centre node; low-score neighbours form a separate summary that can be
        subtracted in fuse_node_edge_representation when neighbour denoising is on.
        """
        ...
        src, dst = edge_index
        num_nodes = h.size(0)
        deg = degree(dst, num_nodes).to(device=h.device, dtype=h.dtype).clamp_min(1.0)
        ...
        learned_logits = self.edge_score_head(edge_hidden).squeeze(-1)
        if self.use_cita_edge_similarity:
            sim_logits = F.cosine_similarity(h_src, h_dst, dim=-1, eps=1e-8)
        else:
            sim_logits = torch.zeros_like(learned_logits)
        edge_logits = (
            self.cita_edge_mlp_weight * learned_logits
            + self.cita_edge_sim_weight * sim_logits
        ) / self.edge_score_temp

        if self.use_edge_score_norm:
            edge_gate = softmax(edge_logits / self.edge_norm_temp, dst, num_nodes=num_nodes)
            low_edge_gate = softmax(-edge_logits / self.noise_gate_temp, dst, num_nodes=num_nodes)
        else:
            edge_gate = torch.sigmoid(edge_logits)
            low_edge_gate = torch.sigmoid(-edge_logits / self.noise_gate_temp)

        if self.edge_norm_with_degree:
            norm = deg[src].pow(-0.5) * deg[dst].pow(-0.5)
        else:
            norm = torch.ones_like(edge_gate)
        useful_weight = torch.nan_to_num(norm * edge_gate, nan=0.0, posinf=0.0, neginf=0.0)
        noise_weight = torch.nan_to_num(norm * low_edge_gate, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def fuse_node_edge_representation(self, h, edge_summary, noise_summary=None, training=False):
        """
        Node update with direct neighbour enhancement and optional weakening.

        High-score neighbours are added as a residual enhancement.  When
        --use_neighbor_denoise is enabled, the separately normalized low-score
        neighbour summary is passed through a gated residual branch and subtracted.
        """
        ...
        if (
            ...
            noise_gate = torch.sigmoid(self.node_noise_gate(noise_input) / self.noise_gate_temp)
    ...
    def encode_representation(self, x, edge_index, training=False):
        ...
        # Front-door decomposition: M keeps a residual path from enhanced X=z
        # so it can mediate X->Y, while U is extracted as a residual-free
        # spurious/environment representation to avoid copying the whole z
        # directly into the context branch.
        ...
        dag_total = z.new_zeros(self.dag_var_dim, self.dag_var_dim)

        layerwise_spurious = None
        if self.use_layerwise_spurious_contexts and layerwise_states:
            layerwise_stack = torch.stack(layerwise_states, dim=0)
            layerwise_spurious = self.spurious_norm(
                self.spurious_adapter(layerwise_stack)
            )
            layerwise_spurious = F.dropout(layerwise_spurious, self.dropout, training=training)
    ...
    def frontdoor_logits_from_contexts(self, z_mediator, z_spurious, contexts):
        """
        Neural front-door adjustment term P(Y | M, U').

        Current-sample U=z_spurious is deliberately not allowed to reach Y.
        Only sampled/averaged context prototypes U' enter the prediction with
        M=z_mediator.  This keeps the path U -> Y blocked except through the
        front-door adjustment contexts.
        """
        ...
        if self.use_dag_mixer:
            # The mixer has a spurious token for API compatibility, but we set
            # it to zero so current U cannot create a direct U->Y path.
            blocked_spurious = torch.zeros_like(mediator_expand)
            fused = self.dag_mixer(
                mediator_expand.reshape(-1, self.d),
                blocked_spurious.reshape(-1, self.d),
    ...
    def forward(self, x, edge_index, training=False):
        ...
        contexts = self.merge_frontdoor_contexts(
            gmm_contexts,
            self.get_saved_spurious_contexts(),
    ...
    def _normalize_edge_distribution(self, weights, dst, num_nodes):
        denom = weights.new_zeros(num_nodes)
        denom.index_add_(0, dst, weights)
        return weights / denom.index_select(0, dst).clamp_min(1e-12)

    def _uniform_edge_prior(self, dst, num_nodes):
        ones = torch.ones_like(dst, dtype=self.ebr_weight.dtype)
        deg = ones.new_zeros(num_nodes)
        deg.index_add_(0, dst, ones)
        return ones / deg.index_select(0, dst).clamp_min(1.0)

    def _edge_prior_logits(self, h_prior, edge_index):
        """Encoder graph-prior scores P_theta(G) from the enhancement scorer."""
        src, dst = edge_index
        deg = degree(dst, h_prior.size(0)).to(device=h_prior.device, dtype=h_prior.dtype).clamp_min(1.0)
        deg_max = torch.log1p(deg).max().clamp_min(1.0)
        edge_feat = self.build_edge_feat(
            h_prior[src],
            h_prior[dst],
        ...
        edge_hidden = self.edge_pair_encoder(edge_feat)
        return self.edge_score_head(edge_hidden).squeeze(-1) / self.edge_score_temp

    def _ebr_edge_logits(self, h, edge_index):
        """
        CGRL-style energy score for an edge <u, v>:
            E(u, v) = -h_v^T W h_u
        This method returns -E(u, v), so larger logits mean lower energy and a
        more likely edge in the reconstructed graph posterior.
        """
        src, dst = edge_index
        h_src = h[src]
        h_dst = h[dst]
    ...
    def _sample_gumbel_like(self, logits):
        """Sample standard Gumbel noise g=-log(-log(u)), u~U(0,1)."""
        eps = self.ebr_gumbel_eps
        u = torch.rand_like(logits).clamp_(eps, 1.0 - eps)
        return -torch.log(-torch.log(u))

    def _ebr_node_distribution(self, logits, tau, training=False, use_gumbel=False):
        """CGRL-style node/G_s posterior with optional Gumbel trick.

        The denominator is over the feature/sample dimension k for each node,
        matching Q_phi(G_s)=softmax((Z_v^(l)+g_k)/tau) in the paper.
        """
        if training and use_gumbel:
            logits = logits + self._sample_gumbel_like(logits)
        probs = F.softmax(logits / tau, dim=-1).clamp_min(1e-12)
        return probs / probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)

    def compute_ebr_node_loss(self, h_causal, h_prior=None, training=False):
        """Node/Gumbel part of CGRL Eq. (4).

        Q_node = GumbelSoftmax(M) during training and Softmax(M) otherwise.
        P_node = Softmax(detached encoder/enhanced representation z).
        This approximates Q_phi(G_s | M) and P_theta(G_s) before concatenating
        with the edge EBM posterior term.
        """
        zero = h_causal.new_zeros(())
        if self.ebr_node_weight <= 0.0 or h_causal is None or h_causal.numel() == 0:
            return zero

        q_node = self._ebr_node_distribution(
            h_causal,
            self.ebr_node_temperature,
            training=training,
            use_gumbel=self.ebr_use_gumbel,
        )

        if h_prior is None or h_prior.numel() == 0 or h_prior.shape != h_causal.shape:
            p_node = torch.full_like(q_node, 1.0 / q_node.size(-1))
        else:
            with torch.no_grad():
                p_node = F.softmax(h_prior.detach() / self.ebr_node_temperature, dim=-1).clamp_min(1e-12)
                p_node = p_node / p_node.sum(dim=-1, keepdim=True).clamp_min(1e-12)

        log_q = q_node.log()
        log_p = p_node.log()
        if self.ebr_kl_direction == 'p_to_q':
            node_kl = (p_node.detach() * (log_p.detach() - log_q)).sum(dim=-1).mean()
        else:
            node_kl = (q_node * (log_q - log_p.detach())).sum(dim=-1).mean()
        return node_kl

    def compute_ebr_loss(self, h_causal, edge_index, h_prior=None, training=False):
        """
        CGRL formula-(4)-style EBR on the causal mediator M.

        L_rec follows the CGRL Eq. (4) idea:

            KL(Q_phi(G_s | M) || P_theta(G_s))

        with Q_phi(G_s | M) approximated by two terms:
        1) a node/Gumbel posterior Q_phi(G_s) = softmax((M + g) / tau),
           where g=-log(-log(u)), u~U(0,1);
        2) an edge EBM posterior Q_phi(u,v) from E(u,v)=-M_v^T W M_u.

        In code we optimize the sum of node KL and edge KL instead of literally
        flattening and concatenating heterogeneous node/edge distributions,
        which is numerically more stable while preserving the paper's intent.
        """
        zero = h_causal.new_zeros(())
        self._last_ebr_bce_loss = zero.detach()
        self._last_ebr_nce_loss = zero.detach()
        self._last_ebr_kl_loss = zero.detach()
        self._last_ebr_node_loss = zero.detach()
        self._last_ebr_edge_loss = zero.detach()
        self._last_ebr_pos_score = zero.detach()
        self._last_ebr_neg_score = zero.detach()
        self._last_ebr_num_pos = 0
        self._last_ebr_num_neg = 0

        if self.lambda_ebr <= 0.0:
            return zero

        node_kl_loss = self.compute_ebr_node_loss(h_causal, h_prior=h_prior, training=training)
        self._last_ebr_node_loss = node_kl_loss.detach()

        if edge_index is None or edge_index.numel() == 0 or self.ebr_kl_weight <= 0.0:
            self._last_ebr_edge_loss = zero.detach()
            self._last_ebr_kl_loss = (self.ebr_node_weight * node_kl_loss).detach()
            return self.ebr_node_weight * node_kl_loss

        num_nodes = h_causal.size(0)
        ...
        pos_logits = self._ebr_edge_logits(h_causal, pos_edge_index)
        ...
        q_edge = self._normalize_edge_distribution(q_edge, pos_dst, num_nodes)
        log_q_edge = q_edge.log()

        if self.ebr_prior == 'encoder' and h_prior is not None:
            with torch.no_grad():
                p_logits = self._edge_prior_logits(h_prior.detach(), pos_edge_index)
                p_edge = softmax(p_logits, pos_dst, num_nodes=num_nodes).clamp_min(1e-12)
                p_edge = self._normalize_edge_distribution(p_edge, pos_dst, num_nodes)
        else:
            p_edge = self._uniform_edge_prior(pos_dst, num_nodes).to(device=h_causal.device, dtype=h_causal.dtype)
            p_edge = p_edge.clamp_min(1e-12)
        log_p_edge = p_edge.log()

        active_dst = torch.unique(pos_dst).numel()
        if self.ebr_kl_direction == 'p_to_q':
            edge_kl_loss = (p_edge.detach() * (log_p_edge.detach() - log_q_edge)).sum() / max(1, int(active_dst))
        else:
            edge_kl_loss = (q_edge * (log_q_edge - log_p_edge.detach())).sum() / max(1, int(active_dst))
        self._last_ebr_edge_loss = edge_kl_loss.detach()
        self._last_ebr_kl_loss = (self.ebr_node_weight * node_kl_loss + self.ebr_kl_weight * edge_kl_loss).detach()

        nce_loss = zero
        bce_loss = zero
        num_neg = int(round(pos_edge_index.size(1) * self.ebr_neg_ratio))
        neg_logits = None
        if num_neg > 0 and (self.ebr_nce_weight > 0.0 or self.ebr_bce_weight > 0.0):
            ...
            if neg_edge_index is not None and neg_edge_index.numel() > 0:
                neg_logits = self._ebr_edge_logits(h_causal, neg_edge_index)
        ...
        if neg_logits is not None and neg_logits.numel() > 0:
            if self.ebr_nce_weight > 0.0:
                pair_count = min(pos_logits.numel(), neg_logits.numel())
                if pair_count > 0:
                    nce_loss = F.softplus(neg_logits[:pair_count] - pos_logits[:pair_count]).mean()
            if self.ebr_bce_weight > 0.0:
                logits = torch.cat([pos_logits, neg_logits], dim=0)
                labels = torch.cat([torch.ones_like(pos_logits), torch.zeros_like(neg_logits)], dim=0)
                bce_loss = F.binary_cross_entropy_with_logits(logits, labels)

        self._last_ebr_nce_loss = nce_loss.detach()
        self._last_ebr_bce_loss = bce_loss.detach()
        return (
            self.ebr_node_weight * node_kl_loss
            + self.ebr_kl_weight * edge_kl_loss
            + self.ebr_nce_weight * nce_loss
            + self.ebr_bce_weight * bce_loss
        )

    def compute_mediator_class_geometry_loss(self, z_mediator, labels):
        """
        Mediator-only class geometry loss for CGRL-style intra/inter behavior.

        This is intentionally implemented as a prototype regularizer rather
        than copying CGRL's replacement losses, so it fits the existing
        front-door objective and leaves the spurious context path untouched.
        """
        ...
        if (
            ...
            or labels is None
            or labels.numel() == 0
            or (self.lambda_proto_compact <= 0.0 and self.lambda_proto_margin <= 0.0)
        ):
            return zero, zero, zero.detach(), zero.detach()

        y = labels.view(-1).to(device=z_mediator.device)
        valid = y >= 0
        if valid.sum() <= 1:
            return zero, zero, zero.detach(), zero.detach()
        ...
        centers = []
        compact_sum = zero
        for idx in range(classes.numel()):
            mask = inverse == idx
            if mask.sum() == 0:
                continue
            cls_h = h[mask]
            center = cls_h.mean(dim=0)
            if self.proto_normalize:
                center = F.normalize(center, dim=0)
            centers.append(center)
            compact_sum = compact_sum + (cls_h - center).pow(2).sum(dim=-1).mean()
        if not centers:
            return zero, zero, zero.detach(), zero.detach()

        compact_loss = compact_sum / float(len(centers))
        centers = torch.stack(centers, dim=0)
        if centers.size(0) <= 1:
            return compact_loss, zero, zero.detach(), zero.detach()

        dist = torch.cdist(centers, centers, p=2)
        off_diag = ~torch.eye(centers.size(0), dtype=torch.bool, device=centers.device)
        pair_dist = dist[off_diag]
        margin_loss = F.relu(self.proto_margin - pair_dist).pow(2).mean()
        return compact_loss, margin_loss, pair_dist.mean().detach(), pair_dist.min().detach()

    @torch.no_grad()
    def update_spurious_context_bank(self, z_spurious, env_probs=None):
        """Save normalized spurious-environment context prototypes for later FD/GMM use."""
        if (not self.save_spu_env_contexts) or z_spurious is None or z_spurious.numel() == 0:
            return
        if env_probs is None or env_probs.numel() == 0 or env_probs.size(-1) != self.num_envs:
            env_probs = self.compute_pseudo_env_probs(z_spurious)
        env_probs = env_probs.detach().clamp_min(0.0)
        values = z_spurious.detach()
        for env_idx in range(self.num_envs):
            weight = env_probs[:, env_idx]
            mass = weight.sum()
            if mass <= 1e-8:
                continue
            context_vec = (weight.unsqueeze(-1) * values).sum(dim=0) / mass.clamp_min(1e-8)
            context_vec = F.normalize(context_vec, dim=0)
            if self.saved_spu_env_context_valid[env_idx]:
                context_vec = self.gamma * self.saved_spu_env_contexts[env_idx] + (1.0 - self.gamma) * context_vec
                context_vec = F.normalize(context_vec, dim=0)
            self.saved_spu_env_contexts[env_idx] = context_vec
            self.saved_spu_env_context_valid[env_idx] = True

    def get_saved_spurious_contexts(self):
        valid = self.saved_spu_env_context_valid.nonzero(as_tuple=False).squeeze(-1)
        if valid.numel() == 0:
            return None
        return self.saved_spu_env_contexts.index_select(0, valid)
    ...
    def compute_independence_loss(self, z_mediator, z_spurious):
        # Legacy pointwise orthogonality diagnostic.  The actual front-door
        # separation loss uses HSIC below because HSIC captures nonlinear
        # statistical dependence between M and U.
    ...
    def _rbf_kernel(self, x):
        ...
        if self.hsic_sigma > 0:
            sigma = x.new_tensor(self.hsic_sigma)
        else:
            with torch.no_grad():
                vals = dist[dist > 0]
                if vals.numel() == 0:
                    sigma = x.new_tensor(1.0)
                else:
                    sigma = torch.sqrt(torch.median(vals)).clamp_min(1e-6)
        gamma = 1.0 / (2.0 * sigma.pow(2).clamp_min(1e-12))
        return torch.exp(-gamma * dist)

    def compute_hsic_loss(self, z_mediator, z_spurious):
        """HSIC(M, U) penalty to discourage a direct M<->U path."""
        if z_mediator.numel() == 0 or z_spurious.numel() == 0:
            return self.A_feat.new_zeros(())
        n = z_mediator.size(0)
        if n <= 1:
        ...
        if max_samples > 0 and n > max_samples:
            idx = torch.randperm(n, device=z_mediator.device)[:max_samples]
            z_mediator = z_mediator.index_select(0, idx)
            z_spurious = z_spurious.index_select(0, idx)
            n = z_mediator.size(0)
        if self.hsic_kernel == 'linear':
            zm = F.normalize(z_mediator, dim=1)
            zu = F.normalize(z_spurious, dim=1)
            k_m = torch.matmul(zm, zm.t())
            k_u = torch.matmul(zu, zu.t())
        else:
            k_m = self._rbf_kernel(z_mediator)
            k_u = self._rbf_kernel(z_spurious)
        k_m = self._center_kernel(k_m)
        k_u = self._center_kernel(k_u)
        return torch.trace(torch.matmul(k_m, k_u)) / float((n - 1) ** 2)
    ...
    def dag_regularization_loss(self, mediator_gate, dag_total):
        # The CIPT-adapter variant deliberately disables the learned DAG
        # structure. Keep the method for training-loop compatibility.
        return mediator_gate.new_zeros(())

    def dag_label_loss(self, dag_vars, labels, train_idx, criterion, args):
        # No DAG-to-label supervision in the CIPT-adapter variant.
        return dag_vars.new_zeros(())
    ...
    def update_spurious_gmm(self, z_spurious, env_probs=None):
        if z_spurious is None or z_spurious.numel() == 0:
            return
        # Always save the spurious-environment context prototypes; these are
        # produced by the spurious adapter and are useful for later front-door
        # context construction/inspection even if GMM sampling is disabled.
        self.update_spurious_context_bank(z_spurious, env_probs)
        if not self.use_spu_gmm:
            return
        means, vars_, valid = self._fit_spurious_gmm_stats(z_spurious, env_probs)
    ...
    def compute_losses(self, data, criterion, args, update_state=False):
        ...
        # Front-door adjustment contexts: sample/average U' from the spurious
        # environment distribution, never the current node's raw U directly.
        gmm_contexts = self.sample_frontdoor_contexts(
            self.sample_gmm_contexts(spu_tr, env_probs_spu, training=True),
            training=True,
        )
        saved_contexts = self.get_saved_spurious_contexts()
        ...
        contexts = self.merge_frontdoor_contexts(gmm_contexts, saved_contexts, global_contexts, layerwise_contexts)
        num_gmm_contexts = 0 if gmm_contexts is None else int(gmm_contexts.size(0))
        num_saved_contexts = 0 if saved_contexts is None else int(saved_contexts.size(0))
        ...
        # Core front-door constraints.
        loss_hsic = self.compute_hsic_loss(med_tr, spu_tr)
        loss_ebr = self.compute_ebr_loss(z_mediator_all, edge_index, h_prior=z_all.detach(), training=True)
        loss_proto_compact, loss_proto_margin, proto_dist_mean, proto_dist_min = (
            self.compute_mediator_class_geometry_loss(med_tr, y_tr)
        )
        ...
        # Compatibility/diagnostic losses kept but not part of the core front-door DAG.
        ...
        total_loss = (
            ...
            + self.lambda_proto_margin * loss_proto_margin
        ...
        return {
            ...
            'loss_ebr_nce': self._last_ebr_nce_loss if self._last_ebr_nce_loss is not None else zero.detach(),
            'loss_ebr_kl': self._last_ebr_kl_loss if self._last_ebr_kl_loss is not None else zero.detach(),
            'loss_ebr_node': self._last_ebr_node_loss if self._last_ebr_node_loss is not None else zero.detach(),
            'loss_ebr_edge': self._last_ebr_edge_loss if self._last_ebr_edge_loss is not None else zero.detach(),
            'loss_node_post': zero.detach(),
            'node_post_entropy': zero.detach(),
            'loss_proto_compact': loss_proto_compact,
            'loss_proto_margin': loss_proto_margin,
            'proto_dist_mean': proto_dist_mean,
            'proto_dist_min': proto_dist_min,
            ...
            'saved_spu_contexts': torch.tensor(float(self.saved_spu_env_context_valid.sum().item()), device=x.device),
            'num_saved_contexts': torch.tensor(float(num_saved_contexts), device=x.device),
