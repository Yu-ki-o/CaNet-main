...
class GraphFrontDoorDAG(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # CGRL-style multi-branch node enhancement. Each branch scores the
        # same edge semantics with its own head, builds an edge-aware neighbor
        # summary, and produces an independent enhancement delta.  The deltas
        # are then averaged/summed/learnedly combined before the residual
        # update.  Setting node_enhance_branches=1 recovers the original
        # single-branch behavior.
        self.node_enhance_branches = max(
            1,
            int(getattr(args, 'node_enhance_branches', getattr(args, 'enhance_branches', 3))),
        )
        self.node_enhance_combine = getattr(args, 'node_enhance_combine', 'mean')
        if self.node_enhance_combine not in ('mean', 'sum', 'learned'):
            self.node_enhance_combine = 'mean'

        self.edge_score_head = nn.Linear(self.d, 1)
        self.extra_edge_score_heads = nn.ModuleList([
            nn.Linear(self.d, 1)
            for _ in range(self.node_enhance_branches - 1)
        ])
        self.edge_summary_norm = nn.LayerNorm(self.d)
        self.extra_edge_summary_norms = nn.ModuleList([
            nn.LayerNorm(self.d)
            for _ in range(self.node_enhance_branches - 1)
        ])
        ...
        self.extra_node_edge_fusers = nn.ModuleList([
            nn.Sequential(
                nn.Linear(self.d * 3, self.d),
                nn.ReLU(),
                nn.Linear(self.d, self.d),
        ...
        self.extra_noise_summary_norms = nn.ModuleList([
            nn.LayerNorm(self.d)
            for _ in range(self.node_enhance_branches - 1)
        ])
        self.node_enhance_branch_logits = Parameter(torch.zeros(self.node_enhance_branches))
    ...
    def reset_parameters(self):
        ...
        for head in self.extra_edge_score_heads:
            head.reset_parameters()
        self.edge_summary_norm.reset_parameters()
        for norm in self.extra_edge_summary_norms:
        ...
        for fuser in self.extra_node_edge_fusers:
            self._reset_module_parameters(fuser)
            nn.init.zeros_(fuser[-1].weight)
            nn.init.zeros_(fuser[-1].bias)
        self.node_edge_norm.reset_parameters()
        self.noise_summary_norm.reset_parameters()
        for norm in self.extra_noise_summary_norms:
            norm.reset_parameters()
        nn.init.zeros_(self.node_enhance_branch_logits)
        self._last_edge_summary_branches = None
        self._last_noise_summary_branches = None
        self._last_edge_gate_branches = None
    ...
    def get_node_enhance_branch_weights(self, device=None, dtype=None):
        if device is None:
            device = self.node_enhance_branch_logits.device
        if dtype is None:
            dtype = self.node_enhance_branch_logits.dtype

        if self.node_enhance_combine == 'learned':
            return torch.softmax(self.node_enhance_branch_logits.to(device=device, dtype=dtype), dim=0)
        if self.node_enhance_combine == 'sum':
            return torch.ones(self.node_enhance_branches, device=device, dtype=dtype)
        return torch.full(
            (self.node_enhance_branches,),
            1.0 / float(self.node_enhance_branches),
            device=device,
            dtype=dtype,
        )
    ...
    def compute_edge_summaries(self, h, edge_index, training=False):
        """
        Build multi-branch useful and low-relevance neighbor summaries.

        For branch k:
            useful_summary_v^k = sum_u norm_uv * g_uv^k       * h_u
            noise_summary_v^k  = sum_u norm_uv * (1 - g_uv^k) * h_u

        The final summary is a branch-combined version of the K summaries. This
        mirrors the K-branch idea in CGRL's RRL block while preserving this
        model's edge feature modes (mul/diff/degree/...).
        """
        self._last_edge_summary_branches = None
        self._last_noise_summary_branches = None
        self._last_edge_gate_branches = None
        ...
        score_heads = [self.edge_score_head] + list(self.extra_edge_score_heads)
        summary_norms = [self.edge_summary_norm] + list(self.extra_edge_summary_norms)
        noise_norms = [self.noise_summary_norm] + list(self.extra_noise_summary_norms)

        useful_summaries = []
        noise_summaries = []
        edge_gates = []
        for score_head, summary_norm, noise_norm in zip(score_heads, summary_norms, noise_norms):
            edge_logits = score_head(edge_hidden).squeeze(-1) / self.edge_score_temp
            ...
            useful_summary = summary_norm(useful_summary)

            noise_summary = h.new_zeros(h.size())
            noise_summary.index_add_(0, dst, noise_weight.unsqueeze(-1) * h_src)
            noise_summary = noise_norm(noise_summary)

            useful_summaries.append(useful_summary)
            noise_summaries.append(noise_summary)
            edge_gates.append(edge_gate)

        useful_stack = torch.stack(useful_summaries, dim=0)
        noise_stack = torch.stack(noise_summaries, dim=0)
        gate_stack = torch.stack(edge_gates, dim=0)
        branch_weights = self.get_node_enhance_branch_weights(
            device=h.device,
            dtype=h.dtype,
        )
        view_shape = (self.node_enhance_branches,) + (1,) * (useful_stack.dim() - 1)
        useful_summary = (branch_weights.view(view_shape) * useful_stack).sum(dim=0)
        noise_summary = (branch_weights.view(view_shape) * noise_stack).sum(dim=0)
        edge_gate = (branch_weights.view(self.node_enhance_branches, 1) * gate_stack).sum(dim=0)

        self._last_edge_summary_branches = useful_stack
        self._last_noise_summary_branches = noise_stack
        self._last_edge_gate_branches = gate_stack
    ...
    def fuse_node_edge_representation(self, h, edge_summary, noise_summary=None, training=False):
        branch_summaries = getattr(self, '_last_edge_summary_branches', None)
        use_branch_fusers = (
            branch_summaries is not None
            and branch_summaries.dim() == 3
            and branch_summaries.size(0) == self.node_enhance_branches
            and branch_summaries.size(1) == h.size(0)
            and branch_summaries.size(2) == h.size(1)
        )

        if use_branch_fusers:
            fusers = [self.node_edge_fuser] + list(self.extra_node_edge_fusers)
            useful_deltas = []
            for branch_idx, fuser in enumerate(fusers):
                edge_summary_k = branch_summaries[branch_idx]
                useful_input = torch.cat([h, edge_summary_k, h * edge_summary_k], dim=-1)
                useful_delta_k = fuser(useful_input)
                useful_delta_k = F.dropout(useful_delta_k, self.dropout, training=training)
                useful_deltas.append(useful_delta_k)
            delta_stack = torch.stack(useful_deltas, dim=0)
            branch_weights = self.get_node_enhance_branch_weights(
                device=h.device,
                dtype=h.dtype,
            )
            useful_delta = (branch_weights.view(self.node_enhance_branches, 1, 1) * delta_stack).sum(dim=0)
        else:
