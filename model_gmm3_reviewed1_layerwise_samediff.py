...
class GraphFrontDoorDAG(BaseGraphFrontDoorDAG):
    ...
    - different path: aggregates center-neighbor contrast/boundary messages.

    The two paths are computed layer by layer from predicted class relations,
    not from ground-truth labels.  The final same-path state is used as
    z_causal, the final different-path state is used as z_spurious, and the
    front-door path directly fuses each node's own (z_causal, z_spurious).
    ...
    def __init__(self, d_in, c, args, device):
        ...
        self.same_diff_hard = bool(getattr(args, 'same_diff_hard', False))
    ...
    def reset_same_diff_parameters(self):
        ...
        for norms in (self.same_path_norms, self.diff_path_norms, self.layer_fuse_norms):
            for norm in norms:
    ...
    def _same_diff_layer(self, h, edge_index, layer_idx, training=False):
        ...
        same_delta = self.same_path_mlps[layer_idx](torch.cat([h, same_summary], dim=-1))
        ...
        diff_delta = self.diff_path_mlps[layer_idx](torch.cat([h, diff_summary], dim=-1))
        ...
        return h_next, z_same, z_diff, same_weight.mean()
    ...
    def encode_representation(self, x, edge_index, training=False):
        ...
        for layer_idx in range(self.num_layers):
            ...
            h, z_causal, z_spurious, same_mean = self._same_diff_layer(
                h,
                edge_index,
                layer_idx,
                training=training,
    ...
    def forward(self, x, edge_index, training=False):
        ...
        contexts = F.normalize(z_spurious, dim=1).unsqueeze(1)
