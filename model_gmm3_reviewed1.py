    ...
    def encode_representation(self, x, edge_index, training=False):
        ...
        if self.use_ica_split:
            (
                ...
                z_mediator,
                z_spurious,
            ) = self.compute_ica_split(z)
            loss_ica_cov, loss_ica_ng, loss_ica_gate, loss_ica_entropy = self.compute_ica_regularization(
                ica_components,
                ica_gate,
            )
            self._last_ica_cov_loss = loss_ica_cov
            self._last_ica_ng_loss = loss_ica_ng
            self._last_ica_gate_loss = loss_ica_gate
            self._last_ica_entropy_loss = loss_ica_entropy
            self._last_ica_gate_mean = ica_gate.mean().detach()

            pad_dim = self.dag_latent_dim - self.ica_dim
            if pad_dim > 0:
                pad = ica_components.new_zeros(ica_components.size(0), pad_dim)
                node_latent = torch.cat([ica_causal, pad], dim=-1)
                edge_latent = torch.cat([ica_spurious, pad], dim=-1)
            else:
                node_latent = ica_causal[:, :self.dag_latent_dim]
                edge_latent = ica_spurious[:, :self.dag_latent_dim]
            dag_vars = torch.cat([node_latent, edge_latent], dim=-1)
            mediator_gate = self.ica_causal_proj.weight.matmul(ica_gate)
            mediator_gate = self._normalize_score(mediator_gate, default_value=float(ica_gate.mean().detach()))
            causal_score = ica_gate.detach()
            pollution_score = (1.0 - ica_gate).detach()
            ...
            self._last_ica_gate_mean = mediator_gate.new_zeros(())
