...
class GraphFrontDoorDAG(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # Enhancement-only node update.  Low-score neighbours are not subtracted
        # from node representations; they simply receive smaller normalized weights
        # in the useful-neighbour enhancement branch.  The denoising arguments are
        # kept for CLI compatibility, but the subtraction path is disabled.
        self.use_neighbor_denoise = False
        self.noise_subtract_alpha = 0.0
        ...
        # Mediator-only semantic posterior consistency.  Unlike the CGRL
        # Gumbel node posterior, this keeps the front-door mediator clean: M is
        # not given a residual z path; it is only softly aligned to a detached
        # low-dimensional posterior derived from the enhanced representation.
        self.lambda_node_post = max(0.0, float(getattr(args, 'lambda_node_post', 0.0)))
        requested_node_post_dim = int(getattr(args, 'node_post_dim', min(32, self.d)))
        self.node_post_dim = max(2, min(requested_node_post_dim, self.d))
        self.node_post_temperature = max(1e-3, float(getattr(args, 'node_post_temperature', 1.0)))
        self.node_post_entropy_weight = max(0.0, float(getattr(args, 'node_post_entropy_weight', 0.0)))
        self.node_post_min_entropy = min(
            max(float(getattr(args, 'node_post_min_entropy', 0.2)), 0.0),
            1.0,
        )
        self.node_post_proj = nn.Sequential(
            nn.Linear(self.d, self.node_post_dim),
            nn.LayerNorm(self.node_post_dim),
        )
        self._last_node_post_entropy = None

        # Lightweight mediator class-geometry regularization.  This adapts the
        # CGRL intra/inter idea to this model without adding CGRL's full loss
        # replacement: compact same-class mediator prototypes and keep class
        # prototypes separated by a margin.
    ...
    def reset_parameters(self):
        ...
        self._reset_module_parameters(self.node_post_proj)
        self._last_ebr_bce_loss = None
        self._last_ebr_nce_loss = None
        self._last_ebr_kl_loss = None
        self._last_ebr_pos_score = None
    ...
    def compute_edge_summaries(self, h, edge_index, training=False):
        """
        Enhancement-only relation-aware neighbour summary.

        useful_summary_v = sum_u norm_uv * alpha_uv * h_u

        alpha_uv is obtained from the learned edge score.  By default it is
        normalized over each destination node's incoming neighbours, matching
        the CGRL/GAT-style neighbour normalization idea.  Low-score neighbours
        are not subtracted from h; they only receive smaller enhancement weights.
        """
        ...
        # Enhancement-only: no low-score-neighbour subtraction branch.
    ...
    def fuse_node_edge_representation(self, h, edge_summary, noise_summary=None, training=False):
        """
        Enhancement-only node update.

        The model only adds relation-aware useful neighbour information to h.
        It deliberately does not subtract low-score-neighbour representations,
        so the pre-adapter representation is enhanced rather than weakened.
        """
    ...
    def compute_node_posterior_loss(self, h_causal, h_prior=None):
        """
        Align M to a detached semantic posterior from z without feeding z into M.

        This is intentionally not the CGRL Gumbel term.  It gives the mediator
        a soft distributional target from the enhanced representation while
        preserving the front-door restriction that raw z should not directly
        enter the mediator path.
        """
        zero = h_causal.new_zeros(())
        self._last_node_post_entropy = zero.detach()
        if self.lambda_node_post <= 0.0 or h_causal is None or h_causal.numel() == 0:
            return zero

        tau = self.node_post_temperature
        q_logits = self.node_post_proj(h_causal) / tau
        log_q = F.log_softmax(q_logits, dim=-1)
        q = log_q.exp().clamp_min(1e-12)

        if h_prior is None or h_prior.numel() == 0 or h_prior.shape != h_causal.shape:
            p = torch.full_like(q, 1.0 / q.size(-1))
        else:
            with torch.no_grad():
                p_logits = self.node_post_proj(h_prior.detach()) / tau
                p = F.softmax(p_logits, dim=-1).clamp_min(1e-12)
                p = p / p.sum(dim=-1, keepdim=True).clamp_min(1e-12)
        log_p = p.log()
        kl_loss = (q * (log_q - log_p.detach())).sum(dim=-1).mean()

        entropy = -(q * log_q).sum(dim=-1).mean()
        entropy_norm = entropy / torch.log(q.new_tensor(float(q.size(-1))).clamp_min(2.0))
        self._last_node_post_entropy = entropy_norm.detach()
        if self.node_post_entropy_weight > 0.0 and self.node_post_min_entropy > 0.0:
            entropy_floor = F.relu(q.new_tensor(self.node_post_min_entropy) - entropy_norm).pow(2)
            kl_loss = kl_loss + self.node_post_entropy_weight * entropy_floor
        return kl_loss

    def compute_mediator_class_geometry_loss(self, z_mediator, labels):
        """
        Small mediator-only class geometry loss.

        `compact` pulls train nodes toward their class prototype in M-space.
        `margin` separates class prototypes.  The spurious branch and context
        bank are untouched, so this does not create a direct U -> Y path.
        """
    ...
    def compute_ebr_loss(self, h_causal, edge_index, h_prior=None, training=False):
        ...
        L_rec = KL(Q_phi(G | M) || P_theta(G)).  Q_phi is the EBM posterior
        obtained from M with edge energy E(u,v)=-M_v^T W M_u. P_theta is a
        detached graph prior from the encoder/enhancement edge scorer, or a
        uniform observed-neighbour prior when requested.
        """
        zero = h_causal.new_zeros(())
        ...
        if self.ebr_kl_direction == 'p_to_q':
            kl_loss = (p_edge.detach() * (log_p_edge.detach() - log_q_edge)).sum() / max(1, int(active_dst))
        else:
            kl_loss = (q_edge * (log_q_edge - log_p_edge.detach())).sum() / max(1, int(active_dst))
        ...
        return self.ebr_kl_weight * kl_loss + self.ebr_nce_weight * nce_loss + self.ebr_bce_weight * bce_loss
    ...
    def compute_losses(self, data, criterion, args, update_state=False):
        ...
        loss_node_post = self.compute_node_posterior_loss(z_mediator_all, h_prior=z_all.detach())
        ...
        total_loss = (
            ...
            + self.lambda_node_post * loss_node_post
        ...
        return {
            ...
            'loss_node_post': loss_node_post,
            'node_post_entropy': self._last_node_post_entropy if self._last_node_post_entropy is not None else zero.detach(),
