...
class GraphFrontDoorDAG(nn.Module):
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # Stable Query-Key routing controls. In stable_qk mode, edge weights are
        ...
        self.stable_qk_lambda = min(max(float(getattr(args, 'stable_qk_lambda', 0.2)), 0.0), 0.49)
        ...
        self.edge_context_alpha = max(0.0, float(getattr(args, 'edge_context_alpha', 0.0)))
        ...
        # available to the front-door context branch through edge_context_alpha.
    ...
    def _get_edge_feat_dim(self, mode):
        ...
        if mode == 'degree':
            return 1
        if mode in ('mul_diff', 'concat', 'anchor_orth', 'stable_qk'):
            return 2 * self.d if mode not in ('anchor_orth', 'stable_qk') else 3
        if mode == 'concat_diff':
            return 3 * self.d
        ...
        raise ValueError(
            ...
            "mul, diff, degree, mul_diff, concat, concat_diff, anchor_orth, stable_qk, "
    ...
    def build_edge_feat(self, h_src, h_dst, deg_src, deg_dst, deg_max):
        ...
        if mode in ('anchor_orth', 'stable_qk'):
            # Placeholder for module initialization compatibility. The actual
            # anchor_orth / stable_qk gates are computed analytically rather
    ...
    def compute_stable_qk_weights(self, h_src, h_dst, anchor_src, anchor_dst, dst, num_nodes):
        ...
        with stable_qk_lambda < 0.5. Thus the edge identity remains closer to
    ...
    def compute_edge_summaries(self, h, edge_index, training=False, anchor=None):
        ...
        if self.edge_feat_mode == 'anchor_orth':
            ...
            useful_value = h_src
            noise_value = h_src
        elif self.edge_feat_mode == 'stable_qk':
            if anchor is None:
                anchor = h.detach()
            anchor_src = anchor[src]
            anchor_dst = anchor[dst]
    ...
    def encode_representation(self, x, edge_index, training=False):
        ...
        if self.edge_context_alpha > 0.0 and edge_context_accum is not None:
            edge_context = self.edge_context_norm(edge_context_accum)
            z_spurious_base = z_spurious_base + self.edge_context_alpha * edge_context
