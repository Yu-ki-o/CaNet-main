...
from model_frontdoor_ciw_dag_nodeigm import GraphFrontDoorNodeIGMDAG
...
def add_nodeigm_dag_args(parser):
    """Arguments for NodeIGM-style causal edge construction."""
    parser.add_argument('--disable_nodeigm_edges', action='store_false', dest='use_nodeigm_edges',
                        help='disable NodeIGM-style causal/environment edge splitting')
    parser.set_defaults(use_nodeigm_edges=True)
    parser.add_argument('--disable_causal_edge_eval', action='store_false', dest='use_causal_edges_for_eval',
                        help='evaluate on the original graph instead of the learned causal subgraph')
    parser.set_defaults(use_causal_edges_for_eval=True)
    parser.add_argument('--disable_env_edge_mixup', action='store_false', dest='use_env_edge_mixup',
                        help='disable environment-edge mixup stability loss')
    parser.set_defaults(use_env_edge_mixup=True)
    parser.add_argument('--nodeigm_hidden_dim', type=int, default=64,
                        help='dimension of z_all used by the causal edge scorer; set this if shape mismatch occurs')
    parser.add_argument('--nodeigm_edge_mlp_hidden', type=int, default=64,
                        help='hidden dimension for edge importance MLP')
    parser.add_argument('--edge_score_threshold', type=float, default=0.5,
                        help='threshold for learned edge importance score')
    parser.add_argument('--degree_threshold', type=float, default=50.0,
                        help='protect edges incident to nodes with degree above this threshold; <=0 disables')
    parser.add_argument('--min_causal_edge_ratio', type=float, default=0.15,
                        help='minimum fraction of edges kept as causal by top-score fallback')
    parser.add_argument('--max_causal_edge_ratio', type=float, default=1.0,
                        help='maximum fraction of edges kept as causal; 1.0 disables upper clipping')
    parser.add_argument('--target_causal_edge_ratio', type=float, default=0.65,
                        help='soft target for the mean edge importance score')
    parser.add_argument('--lambda_edge_ratio', type=float, default=0.01,
                        help='weight for causal edge ratio regularization')
    parser.add_argument('--lambda_edge_label', type=float, default=0.02,
                        help='weight for train-label homophily supervision on edge scores')
    parser.add_argument('--edge_label_min_edges', type=int, default=16,
                        help='minimum labeled train-train edges required to apply edge label loss')
    parser.add_argument('--lambda_envmix', type=float, default=0.1,
                        help='weight for NodeIGM-style environment edge mixup stability loss')
    parser.add_argument('--lambda_envmix_vrex', type=float, default=0.1,
                        help='V-REx variance penalty weight inside env-edge mixup loss')
    parser.add_argument('--envmix_ratios', type=str, default='0.1,1.0',
                        help='comma-separated fractions of environment edges mixed back into causal graph')
...
parser = argparse.ArgumentParser(description='Graph Front-Door CIW-DAG Training Pipeline')
parser_add_main_args(parser)
add_frontdoor_dag_args(parser)
add_ciw_dag_args(parser)
add_nodeigm_dag_args(parser)
...
model = GraphFrontDoorNodeIGMDAG(d, c, args, device).to(device)
...
log_dir = os.path.join('.', 'runs', args.dataset, 'frontdoor_ciw_dag', run_name)
os.makedirs(log_dir, exist_ok=True)
writer = SummaryWriter(log_dir=log_dir)
print(f"[INFO] TensorBoard logging activated. Logs will be saved to: {log_dir}")
print(
    f"[INFO] Training recipe: CIW-DAG + FrontDoor | CIPT schedule: {args.use_cipt_schedule} | "
...
print(
    f"[INFO] NodeIGM-DAG: enabled={args.use_nodeigm_edges} | eval causal edges: {args.use_causal_edges_for_eval} | "
    f"env edge mixup: {args.use_env_edge_mixup} | edge threshold: {args.edge_score_threshold} | "
    f"degree threshold: {args.degree_threshold} | min/max causal ratio: {args.min_causal_edge_ratio}/{args.max_causal_edge_ratio} | "
    f"lambda edge label: {args.lambda_edge_label} | lambda envmix: {args.lambda_envmix} | ratios: {args.envmix_ratios}"
)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if 'loss_edge_ratio' in losses:
            writer.add_scalar('Loss/EdgeRatio', (model.lambda_edge_ratio * losses['loss_edge_ratio']).item(), global_step)
            writer.add_scalar('Loss/EdgeLabel', (model.lambda_edge_label * losses['loss_edge_label']).item(), global_step)
            writer.add_scalar('Loss/EnvEdgeMix', (model.lambda_envmix * losses['loss_envmix']).item(), global_step)
            writer.add_scalar('Loss/EnvEdgeMixVREx', losses['loss_envmix_vrex'].item(), global_step)
            writer.add_scalar('NodeIGM/EdgeScoreMean', losses['edge_score_mean'].item(), global_step)
            writer.add_scalar('NodeIGM/CausalEdgeRatio', losses['causal_edge_ratio'].item(), global_step)
            writer.add_scalar('NodeIGM/HighDegreeEdgeRatio', losses['high_degree_edge_ratio'].item(), global_step)
            writer.add_scalar('NodeIGM/NumCausalEdges', losses['num_causal_edges'].item(), global_step)
            writer.add_scalar('NodeIGM/NumEnvEdges', losses['num_env_edges'].item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"EdgeRatio: {losses.get('causal_edge_ratio', torch.tensor(0.0, device=device)).item():.3f}, "
                f"EdgeScore: {losses.get('edge_score_mean', torch.tensor(0.0, device=device)).item():.3f}, "
