...
from model_gmm3_reviewed1_ebr_norm_fix import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--disable_edge_score_norm', action='store_false', dest='use_edge_score_norm',
                        help='disable per-destination neighbour normalization over learned edge scores')
    parser.set_defaults(use_edge_score_norm=True)
    parser.add_argument('--edge_norm_temp', type=float, default=1.0,
                        help='temperature for per-destination edge-score softmax normalization')
    parser.add_argument('--disable_edge_norm_degree', action='store_false', dest='edge_norm_with_degree',
                        help='do not multiply neighbour-normalized edge weights by the GCN degree coefficient')
    ...
    parser.add_argument('--lambda_ebr', type=float, default=0.05,
                        help='weight of CGRL-aligned energy-based graph reconstruction before DAG projection')
    parser.add_argument('--ebr_temperature', type=float, default=1.0,
                        help='temperature for the EBR bilinear edge energy logits')
    parser.add_argument('--ebr_neg_ratio', type=float, default=1.0,
                        help='number of negative sampled non-edges per positive edge in EBR')
    parser.add_argument('--ebr_max_edges', type=int, default=200000,
                        help='maximum positive edges used by EBR each step; <=0 uses all edges')
    parser.add_argument('--ebr_bce_weight', type=float, default=0.0,
                        help='optional inside-EBR weight of positive/negative edge reconstruction BCE; default 0 because NCE is stabler')
    parser.add_argument('--ebr_nce_weight', type=float, default=1.0,
                        help='inside-EBR weight of pairwise positive-vs-negative energy reconstruction')
    parser.add_argument('--ebr_kl_weight', type=float, default=0.1,
                        help='inside-EBR weight of local edge-posterior/prior alignment')
    parser.add_argument('--ebr_kl_direction', type=str, default='p_to_q', choices=['p_to_q', 'q_to_p'],
                        help='p_to_q treats encoder graph prior as target KL(P||Q); q_to_p uses strict paper-form KL(Q||P)')
    parser.add_argument('--disable_ebr_edge_prior', action='store_false', dest='ebr_use_edge_prior',
                        help='use uniform observed-neighbour prior instead of the learned final edge gate in EBR KL')
    parser.set_defaults(ebr_use_edge_prior=True)
    parser.add_argument('--disable_ebr_prior_detach', action='store_false', dest='ebr_prior_detach',
                        help='allow gradients to flow into the learned edge-gate prior used by EBR KL')
    parser.set_defaults(ebr_prior_detach=True)
    parser.add_argument('--keep_ebr_self_loops', action='store_false', dest='ebr_remove_self_loops',
                        help='keep self-loops in EBR reconstruction instead of removing them')
...
print(
    ...
    f"EBR: lambda={args.lambda_ebr}, temp={args.ebr_temperature}, neg_ratio={args.ebr_neg_ratio}, "
    f"max_edges={args.ebr_max_edges}, bce={args.ebr_bce_weight}, nce={args.ebr_nce_weight}, "
    f"kl={args.ebr_kl_weight}, kl_dir={args.ebr_kl_direction}, "
    f"edge_prior={args.ebr_use_edge_prior}, detach_prior={args.ebr_prior_detach}"
)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"EBR_BCE: {losses['loss_ebr_bce'].item():.4f}, "
