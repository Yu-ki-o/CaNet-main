...
from model_gmm3_reviewed1_graph_cfam_proto import GraphFrontDoorDAG
...
def add_frontdoor_dag_args(parser):
    ...
    parser.add_argument('--proto_num', type=int, default=2,
                        help='number of same-label prototype clusters maintained per class')
    parser.add_argument('--proto_temp', type=float, default=0.2,
                        help='temperature for same-label prototype retrieval')
    parser.add_argument('--proto_gate_temp', type=float, default=1.0,
                        help='temperature for prototype-guided mediator gate')
    parser.add_argument('--proto_momentum', type=float, default=0.9,
                        help='EMA momentum for the class-conditional prototype bank')
    parser.add_argument('--proto_confidence', type=float, default=0.7,
                        help='minimum confidence for unlabeled nodes to join prototype extraction')
    parser.add_argument('--proto_max_nodes_per_class', type=int, default=256,
                        help='maximum confident nodes per class used to refresh prototype clusters each step')
    parser.add_argument('--proto_kmeans_iters', type=int, default=3,
                        help='lightweight k-means iterations used to refine class-conditional prototype clusters')
    parser.add_argument('--proto_inject_weight', type=float, default=0.2,
                        help='residual strength of prototype reference injection into enhanced node states')
    parser.add_argument('--proto_gate_target', type=float, default=0.5,
                        help='target mean mediator gate under prototype guidance')
    parser.add_argument('--lambda_proto_gate', type=float, default=0.0,
                        help='weight of the prototype-guided gate balance loss')
    parser.add_argument('--lambda_proto_align', type=float, default=0.0,
                        help='weight of same-label cross-cluster mediator alignment')
    parser.add_argument('--use_proto_contexts', action='store_true',
                        help='add valid same-label prototype values into the front-door context bank')
    parser.add_argument('--proto_context_weight', type=float, default=1.0,
                        help='scale of same-label prototype contexts in the front-door context bank')
    ...
    parser.add_argument('--eval_pred_mode', type=str, default='blend',
                        ...
                        help='prediction path used only during evaluation/inference')
...
parser = argparse.ArgumentParser(description='Graph Front-Door Prototype-Guided Training Pipeline')
...
if not run_name:
    run_name = (
        f"{current_time}_fd_proto_pg{args.lambda_proto_gate}_pa{args.lambda_proto_align}_fd{args.lambda_fd}"
        f"_warm{args.decomp_warmup_epochs}_ramp{args.intervention_ramp_epochs}"
    )
log_dir = os.path.join('.', 'runs', args.dataset, 'frontdoor_proto', run_name)
os.makedirs(log_dir, exist_ok=True)
writer = SummaryWriter(log_dir=log_dir)
print(f"[INFO] TensorBoard logging activated. Logs will be saved to: {log_dir}")
print(
    f"[INFO] Training recipe: Prototype-guided Graph-CFAM | CIPT schedule: {args.use_cipt_schedule} | "
    ...
    f"ICA split: {args.use_ica_split} (components={args.ica_components}, "
    ...
    f"Proto: num={args.proto_num}, temp={args.proto_temp}, gate_temp={args.proto_gate_temp}, "
    f"conf={args.proto_confidence}, max_cls={args.proto_max_nodes_per_class}, "
    f"inject={args.proto_inject_weight}, gate_lambda={args.lambda_proto_gate}, "
    f"align_lambda={args.lambda_proto_align}, ctx={args.use_proto_contexts} "
    f"(weight={args.proto_context_weight}) | "
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('Loss/ProtoGate', (model.lambda_proto_gate * losses['loss_proto_gate']).item(), global_step)
        writer.add_scalar('Loss/ProtoAlign', (model.lambda_proto_align * losses['loss_proto_align']).item(), global_step)
        ...
        writer.add_scalar('Graph/ProtoGateMean', losses['proto_gate_mean'].item(), global_step)
        writer.add_scalar('Graph/ProtoValidRatio', losses['proto_valid_ratio'].item(), global_step)
        ...
        writer.add_scalar('Graph/NumProtoContexts', losses['num_proto_contexts'].item(), global_step)
        ...
        if epoch % args.display_step == 0:
            msg = (
                ...
                f"ProtoGate: {(model.lambda_proto_gate * losses['loss_proto_gate']).item():.4f}, "
                f"ProtoAlign: {(model.lambda_proto_align * losses['loss_proto_align']).item():.4f}, "
                ...
                f"ProtoGateMean: {losses['proto_gate_mean'].item():.3f}, "
                f"ProtoValid: {losses['proto_valid_ratio'].item():.3f}, "
