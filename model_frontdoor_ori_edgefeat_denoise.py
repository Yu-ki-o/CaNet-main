...
class GraphFrontDoor(nn.Module):
    """
    Paper-style non-DAG front-door model for graph OOD with edge-aware node
    ...
    4) a relation-aware edge summary optionally denoises/enhances node features;
    5) diverse environment-specific spurious contexts are sampled and combined
       with the causal feature through an adaptive augmentation module to
       approximate the front-door intervention.

    This variant intentionally does not include the DAG module. It is meant for
    comparing whether the edge-aware node enhancement from the DAG model is
    useful by itself.
    """
    ...
    def __init__(self, d_in, c, args, device):
        ...
        # Edge-aware node representation enhancement copied from the DAG-core
        # edgefeat denoise model, without the DAG gate/regularizer.
